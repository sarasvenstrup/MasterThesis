"""
ResultsGeneratorSimulationExperiment.py
============================
...
def _eigenvalues_of_M(model, variant: str):
    """Return the drift matrix M and its eigenvalues for the given model variant."""
    if variant == "stable":
...
def _sigma_stats(model, z_cloud, z_extra):
    """Return volatility sigma arrays for the training cloud and an expanded diagnostic set."""
...
def _decode_curves_at_horizons(model, z_paths_np, times, horizons, tenors_np, device, dtype):
    """
    Decode simulated latent paths to swap-rate curves at requested horizons.

    Parameters
    ----------
    model : object
        Trained model with a decode_from_z method.
    z_paths_np : np.ndarray, shape (n_paths, n_times, d)
        Latent state trajectories.
    times : np.ndarray
        Time grid corresponding to the second axis of z_paths_np.
    horizons : list of float
        Horizon years at which to decode curves.
    tenors_np : np.ndarray, shape (n_tenors,)
        Market maturities used as the x-axis for S_hat.
    device : torch.device
    ...
    -------
    dict
        Maps each horizon to a dict with keys: "tau", "mean", "p5", "p25",
        "p75", "p95", "samples", "valid_frac".
    """
    results = {}
...
def main():
    ...
    if LATENT_DIM >= 2:
        ...
        for dd in range(LATENT_DIM):
            ...
            ax.set_ylabel(f"$z_{{{dd+1}}}$", fontsize=13)
    ...
    lines = []
    lines.append("% Auto-generated by ResultsGeneratorSimulationExperiment.py — do not edit by hand.")
    ...
    for col_idx, (arr, c, lbl) in enumerate(col_data_fig3):
        for dd in range(LATENT_DIM):
            ...
            if dd == 0:
                axes[0, col_idx].set_title(lbl, fontsize=13)
    ...
    if LATENT_DIM >= 2:
        ...
        for col_idx, (z_arr, r_arr, z_train, M_curr, c, lbl) in enumerate(col_data_fig5):
            ...
            ax0.set_title(lbl, fontsize=13)
            ax0.set_xlabel("Short rate (%)", fontsize=12)
    ...
    for dd in range(LATENT_DIM):
        ...
        _z_train_abs  = np.abs(_np(z_train_stab)[:, dd])
        level_max     = np.log10(np.max(_z_train_abs))
        ...
        ax.axhline(level_max, color=C_GREY, linestyle="--", linewidth=1.0,
                   label="Training maximum")
        ax.set_title(f"$z_{{{dd+1}}}$ growth", fontsize=13)
        ax.set_xlabel("Years", fontsize=12)
        ax.tick_params(axis="both", labelsize=11)
        if dd == 0:
            ax.set_ylabel(r"$\log_{10}(\mathrm{mean}\;|z_\ell|)$", fontsize=12)
        ax.grid(True, alpha=0.3)
    ...
    try:
        ...
        horizon_labels = {h: f"$t = {h}$ yr" for h in CURVE_HORIZONS}
        ...
        for row, (curves, col_model, lbl_model) in enumerate([
            ...
            for ci, h in enumerate(CURVE_HORIZONS):
                ...
                if row == 0:
                    ax.set_title(horizon_labels[h], fontsize=13)
                ax.tick_params(axis="both", labelsize=11)
                if ci == 0:
    ...
    try:
        ...
        for ax, dim in zip(axes, [2, 3, 4]):
            ...
            ax.set_ylabel("RMSE (bps)", fontsize=13)
            ax.tick_params(axis="both", labelsize=11)
            ax.annotate(f"$\\ell={dim}$", xy=(0.99, 0.97), xycoords="axes fraction",
                        ha="right", va="top", fontsize=13, fontweight="bold", color=col)
            ...
            for ev_label, ev_date in EVENTS.items():
                ...
                if dim == 2:
                    ax.text(d, ROLL_CLIP, ev_label, fontsize=9,
