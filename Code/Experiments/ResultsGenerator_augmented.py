...
# load baseline models for all dims (for all-dim failure modes figure)
_baseline_S_hat_all = {}  # dim -> numpy array of reconstructions
for _bl_dim in [2, 3, 4]:
    _bl_dir  = os.path.join(REPO_ROOT, "Figures", "TrainingResults",
                            f"dim{_bl_dim}_baseline", f"ep{EPOCHS}")
    _bl_path = os.path.join(_bl_dir, f"checkpoint_dim{_bl_dim}_ep{EPOCHS}.pt")
    if os.path.exists(_bl_path):
        _bl_ckpt = torch.load(_bl_path, map_location=device)
        # older checkpoints store config in run_config.json rather than inside the .pt
        if "model_config" in _bl_ckpt:
            _bl_latent_dim = _bl_ckpt["model_config"]["latent_dim"]
            _bl_input_dim  = _bl_ckpt["model_config"].get("input_dim", X_tensor.shape[1])
        else:
            import json as _json
            _bl_run_cfg    = _json.load(open(os.path.join(_bl_dir, "run_config.json")))
            _bl_latent_dim = _bl_run_cfg["latent_dim"]
            _bl_input_dim  = X_tensor.shape[1]
        _bl_m = FullModel(input_dim=_bl_input_dim, latent_dim=_bl_latent_dim).to(device)
        _bl_m.load_state_dict(_bl_ckpt["model_state_dict"])
        _bl_m.eval()
        _bl_list = []
        with torch.no_grad():
            for _i in range(0, X_tensor.shape[0], BATCH_SIZE):
                _xb = X_tensor[_i:_i + BATCH_SIZE].to(device)
                _bl_list.append(_bl_m(_xb).cpu())
        _baseline_S_hat_all[_bl_dim] = torch.cat(_bl_list).numpy()
        print(f"  Loaded baseline dim={_bl_dim}")
    else:
        print(f"  ⚠️  Baseline dim={_bl_dim} checkpoint not found: {_bl_path}")

# ── figure: IS scatter — augmented dim=3 (top) vs baseline dim=2 (bottom) ────
print("\nGenerating combined IS scatter figure (augmented dim=3 vs baseline dim=2)...")
if _baseline_S_hat is not None:
    _bl_rmse_is = np.sqrt(np.mean((X_np_all - _baseline_S_hat) ** 2, axis=1)) * 10_000

    # ── five curve-type masks (based on actual data, same for both panels) ──────
    _iss_deep      = (X_np_all < 0).sum(axis=1) >= 7
    _iss_cross     = (X_np_all[:, :5] < 0).all(axis=1) & (X_np_all[:, -1] > 0) & ~_iss_deep
    _iss_neg_any   = (X_np_all < 0).any(axis=1)
    _iss_inv       = (X_np_all[:, 0] > X_np_all[:, -1]) & ~_iss_neg_any
    _iss_other_neg = _iss_neg_any & ~_iss_deep & ~_iss_cross
    _iss_normal    = ~_iss_neg_any & ~_iss_inv

    _iss_masks = [
        (_iss_normal,    "Normal",          C_PALETTE[2]),
        (_iss_inv,       "Inverted",        "black"),
        (_iss_cross,     "Crossing",        "indianred"),
        (_iss_deep,      "Deeply Negative", "#00FF7F"),   # spring green
        (_iss_other_neg, "Other Negative",  "#E67E22"),   # orange
    ]

    _df_bl_regime = pd.DataFrame({
        "as_of_date": pd.to_datetime(meta["as_of_date"].values),
        "rmse_bps":   _bl_rmse_is,
    })

    fig_iss, (ax_iss_a, ax_iss_b) = plt.subplots(
        2, 1, figsize=(11, 7), sharex=True, sharey=True,
    )

    # Top panel: augmented dim=3
    for _mask, _lbl, _col in _iss_masks:
        _sub = df_regime[_mask]
        if len(_sub) == 0:
            continue
        ax_iss_a.scatter(_sub["as_of_date"], _sub["rmse_bps"],
                         s=4, alpha=0.5, color=_col, marker="o", label=_lbl, zorder=3)
    ax_iss_a.set_ylabel("RMSE (bps)", fontsize=11)
    ax_iss_a.text(0.98, 0.97, r"Augmented ($\ell=3$)",
                  transform=ax_iss_a.transAxes, fontsize=10,
                  ha="right", va="top", color="0.3")
    ax_iss_a.spines["top"].set_visible(False)
    ax_iss_a.spines["right"].set_visible(False)

    # Bottom panel: baseline dim=2
    for _mask, _lbl, _col in _iss_masks:
        _sub = _df_bl_regime[_mask]
        if len(_sub) == 0:
            continue
        ax_iss_b.scatter(_sub["as_of_date"], _sub["rmse_bps"],
                         s=4, alpha=0.5, color=_col, marker="o", label=_lbl, zorder=3)
    ax_iss_b.set_ylabel("RMSE (bps)", fontsize=11)
    ax_iss_b.text(0.98, 0.97, r"Baseline ($\ell=2$)",
                  transform=ax_iss_b.transAxes, fontsize=10,
                  ha="right", va="top", color="0.3")
    ax_iss_b.spines["top"].set_visible(False)
    ax_iss_b.spines["right"].set_visible(False)

    plt.setp(ax_iss_b.xaxis.get_majorticklabels(), rotation=30, ha="right")

    _handles_iss, _labels_iss = ax_iss_a.get_legend_handles_labels()
    fig_iss.legend(_handles_iss, _labels_iss, loc="lower center",
                   bbox_to_anchor=(0.5, 0.01), ncol=5, fontsize=9,
                   frameon=False, markerscale=3)
    fig_iss.tight_layout(rect=[0, 0.05, 1, 1])
    save_fig(fig_iss, "augmented_vs_baseline_is_scatter_dim3")
else:
    print("  ⚠️  Skipping combined IS scatter — baseline dim=2 not loaded")
...
if _baseline_S_hat is None:
    ...
    # CAD 2023-01-31 — fourth panel
    _fm_cad_idx = np.where(
        (meta["ccy"].values == "CAD") &
        (pd.to_datetime(meta["as_of_date"].values) == pd.Timestamp("2023-01-31"))
    )[0]
    if len(_fm_cad_idx) == 0:
        print("  ⚠️  CAD 2023-01-31 not found in dataset")
        _fm_gi_cad = None
    else:
        _fm_gi_cad = _fm_cad_idx[0]

    # ── layout: 2 rows × 2 columns ────────────────────────────────────────────
    fig_fm, axes_fm = plt.subplots(2, 2, figsize=(12, 8), sharey=False)
    ax_fm_tl = axes_fm[0, 0]   # top-left:     EUR 2015-03-31
    ax_fm_tr = axes_fm[0, 1]   # top-right:    EUR 2020-10-30
    ax_fm_bl = axes_fm[1, 0]   # bottom-left:  JPY 2016-09-30
    ax_fm_br = axes_fm[1, 1]   # bottom-right: CAD 2023-01-31
    ...
    def _draw_fm_panel(ax, gi, show_ylabel, show_xlabel, rmse_y, rmse_va,
        ...
        ax.plot(tenors, _fit_b,       color="#2c4f8c", linewidth=1.8,
                label=r"Baseline ($\ell=2$)")

        # set y-limits from data before drawing zero line, so axhline
        # does not force the axis to stretch to include 0
        _ymin = min(_act.min(), _fit_b.min())
        _ymax = max(_act.max(), _fit_b.max())
        _ypad = (_ymax - _ymin) * 0.15
        ax.set_ylim(_ymin - _ypad, _ymax + _ypad)

        ax.axhline(0, color="0.7", linewidth=0.8, linestyle=":")
        ax.set_title(f"{_ccy}  {_date}", fontsize=13)
        ax.text(rmse_x, rmse_y, f"RMSE: {_rmse_bps:.1f} bps",
    ...
    if _fm_gi_eur15 is not None:
        _draw_fm_panel(ax_fm_tl, _fm_gi_eur15, show_ylabel=True,  show_xlabel=False,
                       rmse_y=0.03, rmse_va="bottom", rmse_x=0.03, rmse_ha="left")
    _draw_fm_panel(ax_fm_tr, _fm_gi_deep,      show_ylabel=False, show_xlabel=False, rmse_y=0.97, rmse_va="top")
    _draw_fm_panel(ax_fm_bl, _fm_gi_cross,     show_ylabel=True,  show_xlabel=True,  rmse_y=0.03, rmse_va="bottom")
    if _fm_gi_cad is not None:
        _draw_fm_panel(ax_fm_br, _fm_gi_cad,   show_ylabel=False, show_xlabel=True,  rmse_y=0.97, rmse_va="top")

    _handles_fm, _labels_fm = ax_fm_tl.get_legend_handles_labels()
    fig_fm.legend(_handles_fm, _labels_fm, loc="lower center",
                  bbox_to_anchor=(0.5, -0.06), ncol=2, fontsize=12, frameon=False)
    fig_fm.tight_layout()
    ...
    ax_fm2_bl = axes_fm2[1, 0]
    ax_fm2_br = axes_fm2[1, 1]
    ...
    def _draw_fm_panel_all_dim(ax, gi, show_ylabel, show_xlabel, rmse_y, rmse_va,
        ...
        _all_fits = [_act]
        for _d in [2, 3, 4]:
            if _d in _baseline_S_hat_all:
                _all_fits.append(_baseline_S_hat_all[_d][gi] * 100.0)
        ...
        for _d in [2, 3, 4]:
            if _d in _baseline_S_hat_all:
                ax.plot(tenors, _baseline_S_hat_all[_d][gi] * 100.0,
                        color=_dim_colors[_d], linewidth=1.6,
                        label=r"Baseline " + _dim_labels[_d])
    ...
    if _fm_gi_eur15 is not None:
        _draw_fm_panel_all_dim(ax_fm2_tl, _fm_gi_eur15, show_ylabel=True,  show_xlabel=False,
                               rmse_y=0.03, rmse_va="bottom", rmse_x=0.03, rmse_ha="left")
    _draw_fm_panel_all_dim(ax_fm2_tr, _fm_gi_deep,  show_ylabel=False, show_xlabel=False,
                           rmse_y=0.97, rmse_va="top")
    _draw_fm_panel_all_dim(ax_fm2_bl, _fm_gi_cross, show_ylabel=True,  show_xlabel=True,
                           rmse_y=0.03, rmse_va="bottom")
    if _fm_gi_cad is not None:
        _draw_fm_panel_all_dim(ax_fm2_br, _fm_gi_cad, show_ylabel=False, show_xlabel=True,
                               rmse_y=0.97, rmse_va="top")

    _handles_fm2, _labels_fm2 = ax_fm2_tl.get_legend_handles_labels()
    fig_fm2.legend(_handles_fm2, _labels_fm2, loc="lower center",
                   bbox_to_anchor=(0.5, -0.06), ncol=4, fontsize=11, frameon=False)
    fig_fm2.tight_layout()
    save_fig(fig_fm2, "baseline_fit_failure_modes_all_dim")
...
if _baseline_S_hat is None:
    ...
    # ── figure: worst inverted curves — all three models (3×6) ───────────────
    print("\nGenerating worst inverted curves figure...")
    _inv_mask    = inverted & ~negative          # inverted but not negative
    _inv_indices = np.where(_inv_mask)[0]
    print(f"  Found {len(_inv_indices)} inverted (non-negative) curves")

    _inv_rmse  = _b_rmse[_inv_indices]
    N_INV      = min(18, len(_inv_indices))
    if N_INV == 0:
        print("  ⚠️  No inverted curves found — skipping")
    else:
        _inv_worst = _inv_indices[np.argsort(_inv_rmse)[::-1][:N_INV]]
        _inv_worst = _inv_worst[np.argsort(_inv_worst)]   # sort by dataset index
        print(f"  RMSE range: {_inv_rmse[np.argsort(_inv_rmse)[::-1][0]]:.1f} – "
              f"{_inv_rmse[np.argsort(_inv_rmse)[::-1][N_INV-1]]:.1f} bps")

        fig_wi, axes_wi = plt.subplots(3, 6, figsize=(18, 9), sharey=False)

        for _pi, _gi in enumerate(_inv_worst):
            _ax    = axes_wi[_pi // 6, _pi % 6]
            _act   = X_np_all[_gi] * 10_000.0
            _fit_b = _baseline_S_hat[_gi] * 10_000.0
            _fit_a = S_np_all[_gi] * 10_000.0
            _ccy   = meta["ccy"].values[_gi]
            ...
            if _stable_S_hat is not None:
                ...
                _ax.plot(tenors, _fit_s,   color="#c0392b",       linewidth=1.5,
                         label=r"Stable ($\ell=4$)")
                _rmse_txt = f"B:{_rmse_b:.1f} / A:{_rmse_a:.1f} / S:{_rmse_s:.1f} bps"
            else:
                _rmse_txt = f"B:{_rmse_b:.1f} / A:{_rmse_a:.1f} bps"
        ...
        # Hide unused panels if fewer than 18 curves
        for _pi in range(N_INV, 18):
            axes_wi[_pi // 6, _pi % 6].set_visible(False)

        _handles_wi, _labels_wi = axes_wi[0, 0].get_legend_handles_labels()
        fig_wi.legend(_handles_wi, _labels_wi, loc="lower center",
                      bbox_to_anchor=(0.5, -0.02), ncol=4, fontsize=9, frameon=False)
        fig_wi.suptitle(
            r"Worst-fit inverted curves: baseline ($\ell=2$), augmented ($\ell=3$), stable ($\ell=4$)"
            r" — ranked by baseline RMSE",
            fontsize=11, y=1.01,
        )
        fig_wi.tight_layout()
        save_fig(fig_wi, "baseline_fit_worst_inverted_curves")

    # ── figure: worst curves ranked by augmented dim=3 RMSE (3×6) ────────────
    print("\nGenerating augmented dim=3 worst-fit curves figure...")
    _aug3_rmse  = np.sqrt(np.mean((X_np_all - S_np_all) ** 2, axis=1)) * 10_000
    _aug3_worst = np.argsort(_aug3_rmse)[::-1][:18]
    _aug3_worst = _aug3_worst[np.argsort(_aug3_worst)]   # sort by dataset index
    print(f"  Augmented dim=3 worst RMSE range: "
          f"{_aug3_rmse[np.argsort(_aug3_rmse)[::-1][0]]:.1f} – "
          f"{_aug3_rmse[np.argsort(_aug3_rmse)[::-1][17]]:.1f} bps")

    fig_aw, axes_aw = plt.subplots(3, 6, figsize=(18, 9), sharey=False)

    for _pi, _gi in enumerate(_aug3_worst):
        _ax    = axes_aw[_pi // 6, _pi % 6]
        _act   = X_np_all[_gi] * 10_000.0
        _fit_b = _baseline_S_hat[_gi] * 10_000.0
        _fit_a = S_np_all[_gi] * 10_000.0
        _ccy   = meta["ccy"].values[_gi]
    ...
    _handles_aw, _labels_aw = axes_aw[0, 0].get_legend_handles_labels()
    fig_aw.legend(_handles_aw, _labels_aw, loc="lower center",
                  bbox_to_anchor=(0.5, -0.02), ncol=4, fontsize=9, frameon=False)
    fig_aw.suptitle(
        r"Worst-fit curves: augmented ($\ell=3$), baseline ($\ell=2$), stable ($\ell=4$)"
        r" — ranked by augmented $\ell=3$ RMSE",
        fontsize=11, y=1.01,
    )
    fig_aw.tight_layout()
    save_fig(fig_aw, "augmented_fit_worst_curves")

# ── figure: worst OOS curves — baseline dim=2 (3×6) ─────────────────────────
print("\nGenerating baseline dim=2 OOS worst-fit curves figure...")
_oos_pred_path = os.path.join(
    REPO_ROOT, "Figures", "OOSResults", "Roll",
    "OOS_roll_dim2_baseline",
    _ROLL_SUBDIR_A, f"ep{_ROLL_EPOCHS_A}",
    "predictions_test_all.csv",
)
if not os.path.exists(_oos_pred_path):
    print(f"  ⚠️  OOS predictions not found: {_oos_pred_path}")
else:
    _df_oos_wc = pd.read_csv(_oos_pred_path)
    _df_oos_wc = _df_oos_wc.drop_duplicates(subset=["as_of_date", "ccy"]).reset_index(drop=True)

    _act_cols_wc = [c for c in _df_oos_wc.columns if c.startswith("actual_tenor_")]
    _fit_cols_wc = [c for c in _df_oos_wc.columns if c.startswith("fitted_tenor_")]
    _X_oos_wc = _df_oos_wc[_act_cols_wc].values   # (N, 8)
    _S_oos_wc = _df_oos_wc[_fit_cols_wc].values   # (N, 8)

    _oos_rmse_wc = np.sqrt(np.mean((_X_oos_wc - _S_oos_wc) ** 2, axis=1)) * 10_000
    _oos_worst   = np.argsort(_oos_rmse_wc)[::-1][:18]

    print(f"  OOS worst RMSE range: {_oos_rmse_wc[_oos_worst[0]]:.1f} – "
          f"{_oos_rmse_wc[_oos_worst[-1]]:.1f} bps")

    fig_ow, axes_ow = plt.subplots(3, 6, figsize=(18, 9), sharey=False)

    for _pi, _ri in enumerate(_oos_worst):
        _ax    = axes_ow[_pi // 6, _pi % 6]
        _act   = _X_oos_wc[_ri] * 10_000.0
        _fit   = _S_oos_wc[_ri] * 10_000.0
        _rmse  = _oos_rmse_wc[_ri]
        _ccy   = _df_oos_wc["ccy"].iloc[_ri]
        _date  = pd.to_datetime(_df_oos_wc["as_of_date"].iloc[_ri]).strftime("%Y-%m-%d")

        _ax.plot(tenors, _act, "o-", color="black",   linewidth=1.5,
                 markersize=3, label="Actual")
        _ax.plot(tenors, _fit,       color="#2c4f8c", linewidth=1.5,
                 label=r"Baseline ($\ell=2$)")
        _ax.axhline(0, color="0.7", linewidth=0.8, linestyle=":")
        _ax.set_title(f"{_ccy}  {_date}", fontsize=8)
        _ax.text(0.97, 0.97, f"{_rmse:.1f} bps",
                 transform=_ax.transAxes, fontsize=6.5,
                 ha="right", va="top", color="0.4")
        _ax.tick_params(labelsize=7)
        _ax.spines["top"].set_visible(False)
    ...
    _handles_ow, _labels_ow = axes_ow[0, 0].get_legend_handles_labels()
    fig_ow.legend(_handles_ow, _labels_ow, loc="lower center",
                  bbox_to_anchor=(0.5, -0.02), ncol=2, fontsize=9, frameon=False)
    fig_ow.suptitle(
        r"Worst OOS fits: baseline ($\ell=2$) — ranked by OOS RMSE",
        fontsize=11, y=1.01,
    )
    fig_ow.tight_layout()
    save_fig(fig_ow, "baseline_oos_worst_curves")

# ── figure: OOS crossover curve reconstruction — baseline dim=2 + stable dim=4
# Crossover: first 5 tenors (indices 0–4) all negative, last tenor (index 7) positive
print("\nGenerating OOS crossover curves figure (baseline dim=2 + stable dim=4)...")

_oos_pred_path_st4 = os.path.join(
    REPO_ROOT, "Figures", "OOSResults", "Roll",
    "OOS_roll_dim4_stable",
    _ROLL_SUBDIR_A, f"ep{_ROLL_EPOCHS_A}",
    "predictions_test_all.csv",
)

if not os.path.exists(_oos_pred_path):
    print(f"  ⚠️  Baseline OOS predictions not found: {_oos_pred_path}")
else:
    _df_cross_oos = pd.read_csv(_oos_pred_path)
    _df_cross_oos = _df_cross_oos.drop_duplicates(
        subset=["as_of_date", "ccy"]
    ).reset_index(drop=True)

    _act_cols_cr = [c for c in _df_cross_oos.columns if c.startswith("actual_tenor_")]
    _fit_cols_cr = [c for c in _df_cross_oos.columns if c.startswith("fitted_tenor_")]
    _X_cr = _df_cross_oos[_act_cols_cr].values   # (N, 8)
    _S_cr = _df_cross_oos[_fit_cols_cr].values   # (N, 8)

    # load stable dim=4 fitted values, keyed by (as_of_date, ccy)
    _st4_fit_lookup = {}
    if os.path.exists(_oos_pred_path_st4):
        _df_st4 = pd.read_csv(_oos_pred_path_st4)
        _df_st4 = _df_st4.drop_duplicates(subset=["as_of_date", "ccy"]).reset_index(drop=True)
        _fit_cols_st4 = [c for c in _df_st4.columns if c.startswith("fitted_tenor_")]
        for _, _row in _df_st4.iterrows():
            _key = (_row["as_of_date"], _row["ccy"])
            _st4_fit_lookup[_key] = _row[_fit_cols_st4].values.astype(float)
        print(f"  Loaded stable dim=4 OOS predictions ({len(_df_st4)} rows)")
    else:
        print(f"  ⚠️  Stable dim=4 OOS predictions not found: {_oos_pred_path_st4}")

    # crossover mask: short end negative, long end positive
    _cross_mask_oos = ((_X_cr[:, :5] < 0).all(axis=1)) & (_X_cr[:, -1] > 0)
    _cross_idx_oos  = np.where(_cross_mask_oos)[0]
    print(f"  Found {len(_cross_idx_oos)} crossover curves in OOS test set")

    if len(_cross_idx_oos) == 0:
        print("  ⚠️  No crossover curves found in OOS — skipping")
    else:
        _cross_rmse_oos = np.sqrt(
            np.mean((_X_cr[_cross_idx_oos] - _S_cr[_cross_idx_oos]) ** 2, axis=1)
        ) * 10_000

        # rank by baseline RMSE descending; cap at 18 panels
        _N_cr   = min(18, len(_cross_idx_oos))
        _cr_ord = _cross_idx_oos[np.argsort(_cross_rmse_oos)[::-1][:_N_cr]]

        _n_rows_cr = (_N_cr + 5) // 6
        _n_cols_cr = min(6, _N_cr)
        fig_cr, axes_cr = plt.subplots(
            _n_rows_cr, _n_cols_cr,
            figsize=(3 * _n_cols_cr, 3 * _n_rows_cr),
            sharey=False,
        )
        if _n_rows_cr == 1:
            axes_cr = axes_cr[np.newaxis, :]

        for _pi, _ri in enumerate(_cr_ord):
            _ax   = axes_cr[_pi // _n_cols_cr, _pi % _n_cols_cr]
            _act  = _X_cr[_ri] * 10_000.0
            _fit_b = _S_cr[_ri] * 10_000.0
            _rmse_b = float(np.sqrt(np.mean((_act - _fit_b) ** 2)))
            _ccy  = _df_cross_oos["ccy"].iloc[_ri]
            _date = pd.to_datetime(
                _df_cross_oos["as_of_date"].iloc[_ri]
            ).strftime("%Y-%m-%d")
            ...
            # overlay stable dim=4 if available for this observation
            _key = (_df_cross_oos["as_of_date"].iloc[_ri], _ccy)
            _rmse_txt = f"B:{_rmse_b:.1f}"
            if _key in _st4_fit_lookup:
                _fit_s  = _st4_fit_lookup[_key] * 10_000.0
                _rmse_s = float(np.sqrt(np.mean((_act - _fit_s) ** 2)))
                _ax.plot(tenors, _fit_s, color="#c0392b", linewidth=1.5,
                         label=r"Stable ($\ell=4$)")
                _rmse_txt += f" / S:{_rmse_s:.1f}"
            _rmse_txt += " bps"
            ...
            if _pi % _n_cols_cr == 0:
                _ax.set_ylabel("Swap rate (bps)", fontsize=7)
            if _pi // _n_cols_cr == _n_rows_cr - 1:
                _ax.set_xlabel("Tenor (years)", fontsize=7)

        for _pi in range(_N_cr, _n_rows_cr * _n_cols_cr):
            axes_cr[_pi // _n_cols_cr, _pi % _n_cols_cr].set_visible(False)

        _handles_cr, _labels_cr = axes_cr[0, 0].get_legend_handles_labels()
        fig_cr.legend(_handles_cr, _labels_cr, loc="lower center",
                      bbox_to_anchor=(0.5, -0.02), ncol=3, fontsize=9, frameon=False)
        fig_cr.suptitle(
            r"OOS crossover curves: baseline ($\ell=2$) and stable ($\ell=4$)"
            r" — short end $<0$, long end $>0$, ranked by baseline OOS RMSE",
            fontsize=11, y=1.01,
        )
        fig_cr.tight_layout()
        save_fig(fig_cr, "baseline_oos_crossover_curves")
...
if _baseline_S_hat is None:
    ...
    fig_fma, axes_fma = plt.subplots(2, 2, figsize=(12, 8), sharey=False)
    ax_fma_tl = axes_fma[0, 0]   # EUR 2015-03-31
    ax_fma_tr = axes_fma[0, 1]   # EUR 2020-10-30
    ax_fma_bl = axes_fma[1, 0]   # JPY 2016-09-30
    ax_fma_br = axes_fma[1, 1]   # CAD 2023-01-31
    ...
    def _draw_fma_panel(ax, gi, show_ylabel, show_xlabel, rmse_y, rmse_va,
        ...
        ax.plot(tenors, _act,   "o-", color="black", linewidth=1.8,
                markersize=5, label="Actual")
        ax.plot(tenors, _fit_b,       color="black",  linewidth=1.8,
        ...
        for _d in [2, 3, 4]:
            ...
            ax.plot(tenors, _fit_d, color=_fma_colors[_d], linewidth=1.8,
        ...
        # set y-limits from data before drawing zero line so axhline
        # does not force the axis to stretch to include 0
        _all_vals = np.concatenate([_act, _fit_b] + [
            _fma_aug_hats[_d][gi] * 100.0 for _d in [2, 3, 4] if _d in _fma_aug_hats
        ])
        _ymin = _all_vals.min()
        _ymax = _all_vals.max()
        _ypad = (_ymax - _ymin) * 0.15
        ...
        ax.axhline(0, color="0.7", linewidth=0.8, linestyle=":")
        ax.set_title(f"{_ccy}  {_date}", fontsize=13)
        ax.text(rmse_x, rmse_y, _rmse_txt,
    ...
    if _fm_gi_eur15 is not None:
        _draw_fma_panel(ax_fma_tl, _fm_gi_eur15, show_ylabel=True,  show_xlabel=False,
                        rmse_y=0.03, rmse_va="bottom", rmse_x=0.03, rmse_ha="left")
    _draw_fma_panel(ax_fma_tr, _fm_gi_deep,  show_ylabel=False, show_xlabel=False, rmse_y=0.97, rmse_va="top")
    _draw_fma_panel(ax_fma_bl, _fm_gi_cross, show_ylabel=True,  show_xlabel=True,  rmse_y=0.03, rmse_va="bottom")
    if _fm_gi_cad is not None:
        _draw_fma_panel(ax_fma_br, _fm_gi_cad,   show_ylabel=False, show_xlabel=True,  rmse_y=0.97, rmse_va="top")

    _handles_fma, _labels_fma = ax_fma_tl.get_legend_handles_labels()
...
# ─────────────────────────────────────────────────────────────────────────────
# Q4c-style: OOS RMSE by curve regime — model comparison table
# Columns: Baseline ℓ=2 | Augmented ℓ=2 | Augmented ℓ=3 | Augmented ℓ=4
# Rows:    Normal / Inverted / Negative  ×  {N, IS RMSE, OOS RMSE}
#          + Average IS RMSE / OOS RMSE at the bottom
# All values are across ALL currencies (the "All" aggregate).
# ─────────────────────────────────────────────────────────────────────────────
print("\n── Q4c-style regime comparison across models ──")

_ROLL_ROOT   = os.path.join(REPO_ROOT, "Figures", "OOSResults", "Roll")
_ROLL_SUFFIX = os.path.join(_ROLL_SUBDIR_A, f"ep{_ROLL_EPOCHS_A}")

_q4c_models = [
    ("Baseline l=2",   os.path.join(_ROLL_ROOT, "OOS_roll_dim2_baseline",          _ROLL_SUFFIX)),
    ("Augmented l=2",  os.path.join(_ROLL_ROOT, "OOS_roll_dim2_augmented_input",   _ROLL_SUFFIX)),
    ("Augmented l=3",  os.path.join(_ROLL_ROOT, "OOS_roll_dim3_augmented_input",   _ROLL_SUFFIX)),
    ("Augmented l=4",  os.path.join(_ROLL_ROOT, "OOS_roll_dim4_augmented_input",   _ROLL_SUFFIX)),
]

_q4c_groups = [
    ("Normal",   lambda df: ~df["inverted"] & ~df["negative"]),
    ("Inverted", lambda df:  df["inverted"] & ~df["negative"]),
    ("Negative", lambda df: ~df["inverted"] &  df["negative"]),
]

def _load_regime_df(folder, split):
    """Load predictions_{split}_all.csv and return a regime DataFrame.

    Some rolling CSVs were generated with duplicate rows (same as_of_date × ccy
    written multiple times with identical fitted values).  We deduplicate on
    (as_of_date, ccy) before computing statistics so that N counts are correct.
    """
    path = os.path.join(folder, f"predictions_{split}_all.csv")
    if not os.path.exists(path):
        return None
    _df = pd.read_csv(path)
    # Drop exact duplicates caused by multi-run concatenation in rolling script
    _df = _df.drop_duplicates(subset=["as_of_date", "ccy"]).reset_index(drop=True)
    _act_cols = [c for c in _df.columns if c.startswith("actual_tenor_")]
    _fit_cols = [c for c in _df.columns if c.startswith("fitted_tenor_")]
    _X = _df[_act_cols].values
    _S = _df[_fit_cols].values
    _rmse = np.sqrt(np.mean((_X - _S) ** 2, axis=1)) * 10_000
    return pd.DataFrame({
        "rmse_bps": _rmse,
        "inverted": _X[:, 0] > _X[:, -1],
        "negative": (_X < 0).any(axis=1),
    })

def _fmt_n(v): return str(int(v)) if pd.notna(v) and v > 0 else "---"
def _fmt_r(v): return f"{v:.1f}" if pd.notna(v) else "---"

_q4c_col_data = {}   # label -> {"oos": df, "is": df}
for _q4c_lbl, _q4c_folder in _q4c_models:
    _df_oos_q = _load_regime_df(_q4c_folder, "test")
    _df_is_q  = _load_regime_df(_q4c_folder, "train")
    if _df_oos_q is None:
        print(f"  ⚠️  {_q4c_lbl}: predictions_test_all.csv not found — skipping")
        continue
    _q4c_col_data[_q4c_lbl] = {"oos": _df_oos_q, "is": _df_is_q}
    print(f"  Loaded {_q4c_lbl}: {len(_df_oos_q)} OOS obs")

if _q4c_col_data:
    _q4c_rows = []
    _col_labels = list(_q4c_col_data.keys())

    for _grp_lbl, _mask_fn in _q4c_groups:
        # N row (OOS count)
        _row_n = {"Curve Variant": _grp_lbl, "Metric": "N"}
        for _lbl in _col_labels:
            _doo = _q4c_col_data[_lbl]["oos"]
            _m   = _mask_fn(_doo)
            _row_n[_lbl] = _fmt_n(_m.sum())
        _q4c_rows.append(_row_n)

        # IS RMSE row
        _row_is = {"Curve Variant": _grp_lbl, "Metric": "IS RMSE"}
        for _lbl in _col_labels:
            _dis = _q4c_col_data[_lbl]["is"]
            if _dis is None:
                _row_is[_lbl] = "---"
            else:
                _m  = _mask_fn(_dis)
                _sub = _dis.loc[_m, "rmse_bps"]
                _row_is[_lbl] = _fmt_r(_sub.mean()) if len(_sub) > 0 else "---"
        _q4c_rows.append(_row_is)

        # OOS RMSE row
        _row_oos = {"Curve Variant": _grp_lbl, "Metric": "OOS RMSE"}
        for _lbl in _col_labels:
            _doo = _q4c_col_data[_lbl]["oos"]
            _m   = _mask_fn(_doo)
            _sub = _doo.loc[_m, "rmse_bps"]
            _row_oos[_lbl] = _fmt_r(_sub.mean()) if len(_sub) > 0 else "---"
        _q4c_rows.append(_row_oos)

    # Average rows (all curves, all currencies)
    _row_avg_is  = {"Curve Variant": "Average", "Metric": "IS RMSE"}
    _row_avg_oos = {"Curve Variant": "Average", "Metric": "OOS RMSE"}
    for _lbl in _col_labels:
        _dis = _q4c_col_data[_lbl]["is"]
        _row_avg_is[_lbl]  = _fmt_r(_dis["rmse_bps"].mean()) if _dis is not None else "---"
        _row_avg_oos[_lbl] = _fmt_r(_q4c_col_data[_lbl]["oos"]["rmse_bps"].mean())
    _q4c_rows.append(_row_avg_is)
    _q4c_rows.append(_row_avg_oos)

    _q4c_tbl = pd.DataFrame(_q4c_rows, columns=["Curve Variant", "Metric"] + _col_labels)
    print("\n" + _q4c_tbl.to_string(index=False))

    _q4c_out = os.path.join(FIGURES_OUT, "Q4c_regime_model_comparison.csv")
    _q4c_tbl.to_csv(_q4c_out, index=False)
    print(f"  Saved: {_q4c_out}")
