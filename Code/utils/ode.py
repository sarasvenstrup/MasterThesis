...
def solve_AB_rk38(
    ...
    RK4 3/8 rule along tau grid, with linear interpolation of (alpha,beta,gamma)
    inside each interval [tau_i, tau_{i+1}].

    tau:   (N,)
    alpha,beta,gamma: (batch, N)
    returns: A,B each (batch, N)

    NOTE: implemented without in-place writes to keep autograd happy.
    """
    if tau.ndim != 1:
        raise ValueError("tau must be 1D (N,)")
    if alpha.shape != beta.shape or alpha.shape != gamma.shape:
        raise ValueError("alpha,beta,gamma must have same shape (batch,N)")
    ...
    # coefficient interpolation on each interval
    def coeff(i: int, theta):
        th = torch.as_tensor(theta, device=alpha.device, dtype=alpha.dtype)
    ...
    def rhs(i: int, theta, Acur: torch.Tensor, Bcur: torch.Tensor):
        a, b, g = coeff(i, theta)
        dB = a * Bcur + b
        dA = g * (Bcur ** 2)
        return dA, dB
    ...
    # state (batch,1)
    Acur = torch.zeros(batch, 1, device=alpha.device, dtype=alpha.dtype)
    Bcur = torch.zeros(batch, 1, device=alpha.device, dtype=alpha.dtype)
    ...
    for i in range(N - 1):
        ...
        k1A, k1B = rhs(i, 0.0,       Acur,                      Bcur)
        k2A, k2B = rhs(i, 1.0/3.0,   Acur + h/3.0 * k1A,        Bcur + h/3.0 * k1B)
        k3A, k3B = rhs(i, 2.0/3.0,   Acur + 2*h/3.0 * k2A,      Bcur + 2*h/3.0 * k2B)
        k4A, k4B = rhs(i, 1.0,
                       Acur + h * (-k1A + k2A + k3A),
                       Bcur + h * (-k1B + k2B + k3B))
