...
class HConstStable(nn.Module):
    """
    Constant lower-triangular diffusion factor L.

    Returns the same L for all z.
    Covariance is Sigma = L L^T.

    Purpose:
      - isolate the effect of state-dependent diffusion
      - make H close to a Gaussian / A0-style baseline
    """
    ...
    def __init__(
        ...
        self.raw_diag = nn.Parameter(torch.full((self.d,), raw_init))
        self.raw_off = nn.Parameter(torch.zeros(self.n_off))
    ...
    def forward(self, z: torch.Tensor, return_raw: bool = False):
        ...
        log_diag = self.log_sigma_min + self.log_sigma_range * torch.sigmoid(self.raw_diag)
        diag = torch.exp(log_diag)  # (d,)

        off = self.offdiag_max * torch.tanh(self.raw_off)  # (n_off,)

        L0 = torch.zeros(self.d, self.d, device=z.device, dtype=z.dtype)
        L0[torch.arange(self.d), torch.arange(self.d)] = diag.to(dtype=z.dtype, device=z.device)
        if self.n_off > 0:
            L0[self.tril_i, self.tril_j] = off.to(dtype=z.dtype, device=z.device)

        L = L0.unsqueeze(0).expand(B, -1, -1)
