...
Workflow:
    ...
    2. python ResultsGeneratorPricingComparison.py      # fast — loads CSV, plots, metrics

Usage:
    python ResultsGeneratorPricingComparison.py
    python ResultsGeneratorPricingComparison.py --model_prices path/to/model_prices.csv
"""
...
def load_market_vols(excel_path: Path, sheet_name: str = 'EUR SwapNVol OIS') -> pd.DataFrame:
    ...
    Returns
    -------
    pd.DataFrame with columns: date, expiry (int, years), tenor (int, years),
...
def write_latex_table(df_cells: pd.DataFrame,
    ...
    """
    lines: List[str] = []
    lines.append("% Auto-generated by Code/Pricing/ResultsGeneratorPricingComparison.py — do not edit by hand.")
...
def main():
    ...
    # Output directory tagged by CCY_DIM_MODEL; overwritten on re-runs.
