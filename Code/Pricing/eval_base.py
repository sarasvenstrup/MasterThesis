...
"""
Evaluation of the pretrained reconstruction model (dim4_stable/ep5000)
with NO pricing adjustment:

    K_price(z) = K_base(z)    (physical drift, unchanged)
    sigma_scale = 1.0         (no diffusion scaling)

This is the baseline row in the pricing experiment table.
It answers: how badly does the reconstruction model price swaptions
before any calibration?

Output -> Figures/pricing/eval_base/
"""
...
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_base")
...
model.eval()
print("Base model loaded.  k_override=None  sigma_scale=None (=> 1.0)")
...
def price_cell(date, expiry, tenor):
    ...
    with torch.no_grad():
        ...
        # No k_override → uses model.K (physical drift K^P)
        # No sigma_scale → defaults to 1.0
...
print(f"\nk_override:  None  (K^P base dynamics)")
print(f"sigma_scale: 1.0   (no diffusion scaling)")
...
for split_label2, split_key2, label_suffix in [
    ...
    lines = [
        ...
        (rf"\caption{{Per-cell ATM straddle vol errors: base model, no pricing adjustment "
         rf"({split_label2}). EUR.}}"),
        rf"\label{{tab:base_per_cell_{label_suffix}}}",
        r"\small",
...
fig.suptitle("Base Model (no adjustment): Model vs Market Straddle Vol",
    ...
    for j, t in enumerate(TENOR_VALS):
        ...
        if s:
            ax.text(0.04, 0.96, f"MAE={s['mae_bp']:.0f}bp", transform=ax.transAxes,
                    fontsize=7, va="top",
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))
fig.suptitle("Base Model (no adjustment): Vol error over time (amber=test)",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0, 0, 1, 0.96])
...
for split_label3, split_key3, fname3 in [
    ...
    ax.set_title(f"Base Model: Vol MAE per Cell (bp), {split_label3}")
    plt.tight_layout()
...
print(f"  k_override:    None  (K^P physical dynamics)")
print(f"  sigma_scale:   1.0   (no scaling)")
