# ==================== Lambda v2 Evaluation ====================
"""
Evaluation for Training_lambda_v2.py checkpoint.

Key fix vs old eval: ATM strike = FORWARD swap rate, not spot swap rate.
  F_0 = (P(0,expiry) - P(0,expiry+tenor)) / A_fwd

Outputs -> Figures/TrainingResults/dim4_lambda_v2/ep1000/eval/
"""
...
lines = [
    ...
    (r"\caption{Per-cell ATM vol errors: $\Lambda$-MPR v2 (correct forward strike, "
     r"variance vol loss, trainable $\sigma$-scale). EUR test set.}"),
    r"\label{tab:lambda_v2_per_cell}",
    r"\small",
...
with open(os.path.join(OUT_DIR, "tab_lambda_v2_per_cell.tex"), "w") as f:
    f.write("\n".join(lines))
print("Saved: tab_lambda_v2_per_cell.tex")
...
for i, e in enumerate(EXPIRY_VALS):
    for j, t in enumerate(TENOR_VALS):
        ...
        s = cell_stats(e, t)
        if s:
            ax.text(0.05, 0.95, f"MAE={s['mae_bp']:.0f}bp", transform=ax.transAxes,
                    fontsize=7, va="top",
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))
fig.suptitle(r"$\Lambda$-MPR v2: Model vs Market ATM Vol (correct fwd strike)",
    ...
    for j, t in enumerate(TENOR_VALS):
        ...
        if s:
            ax.text(0.04, 0.96, f"MAE={s['mae_bp']:.0f}bp", transform=ax.transAxes,
                    fontsize=7, va="top",
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))
fig.suptitle(r"$\Lambda$-MPR v2: Vol error over time, EUR test set",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0,0,1,0.96])
...
ax.set_title(r"$\Lambda$-MPR v2: Vol MAE per Cell (bp)"); plt.tight_layout()
...
for i, e in enumerate(EXPIRY_VALS):
    for j, t in enumerate(TENOR_VALS):
        ...
        ax.text(0.04, 0.96, f"mean={sub['forward_bias_bp'].mean():+.0f}bp",
                ...
                bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))
fig.suptitle(r"$\Lambda$-MPR v2: Forward bias (target: 0 bp)",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0,0,1,0.96])
fig.savefig(os.path.join(OUT_DIR, "fig_forward_bias_timeseries.png"), dpi=150, bbox_inches="tight")
plt.close(fig)
