# ==================== Lambda MPR Evaluation — Control Variate Correction ====================
"""
Same as eval_lambda_mpr.py but with a mean-correction (control variate) applied to the
Monte Carlo paths before pricing.

Theory
------
Under the swap measure A, F_T is a martingale:  E^A[F_T] = F_0.
In the simulation under Q the MC estimator gives:

    F_bar = sum_i(D_i * A_i * F_i) / sum_i(D_i * A_i)   ~ E^A[F_T]

The raw forward bias is  (F_bar - F_0).  We correct every path by shifting:

    F_T_adj^i  =  F_T^i  -  F_bar  +  F_0

so that the corrected estimator satisfies  E^A[F_T_adj] = F_0  exactly (by construction),
while leaving the distribution shape (variance, skewness, kurtosis) unchanged.
The correction is a valid control variate: it subtracts a zero-mean term from the payoff.

Outputs -> Figures/TrainingResults/dim4_stable_lambda_mpr/ep1000/eval_cv/
  per_cell_final.csv              raw (date, expiry, tenor) results
  tab_lambda_per_cell.tex         LaTeX table: per-cell MAE/RMSE/bias/fwd-bias
  fig_vol_surface.png             3x3 scatter: model vs market vol
  fig_vol_error_timeseries.png    3x3 time-series: vol error over dates
  fig_vol_heatmap.png             heatmap of vol MAE per cell
  fig_forward_bias_timeseries.png forward bias over time (should be ~0)
  fig_raw_bias_timeseries.png     raw (pre-correction) forward bias for diagnostics
"""
...
OUT_DIR       = os.path.join(CKPT_DIR, "eval_cv")
...
print("Control variate correction: ON  (E^A[F_T] = F_0 enforced per swaption)")
...
# ================================================================
# Pricing function — with mean-correction control variate
# ================================================================
...
def price_cell(date, expiry, tenor):
    ...
    # ---- Mean-correction control variate --------------------------------
    # Compute annuity-discounted model forward  E^A[F_T] = E^Q[D*A*F] / E^Q[D*A]
    w        = D_k * A_T
    F_bar    = (w * F_T).sum() / w.sum()           # MC estimate of E^A[F_T]
    raw_fwd_bias_bp = float((F_bar - F0) / A0 * 1e4)  # diagnostic: bias before correction
    F_T      = F_T - F_bar + F0                    # shift → E^A[F_T_adj] = F0 exactly
    # ---------------------------------------------------------------------
    ...
    forward_bias_bp = (V_pay - V_rec) / A0 * 1e4   # should be ~0 by construction
    ...
    return {
        ...
        "raw_fwd_bias_bp": raw_fwd_bias_bp,
        "path_frac":       path_frac,
    }
...
for counter, (_, row) in enumerate(combos_test.iterrows()):
    ...
    rows.append({
        ...
        "raw_fwd_bias_bp": result["raw_fwd_bias_bp"],
...
def cell_stats(expiry, tenor):
    ...
    return {
        ...
        "raw_fwd_bias_bp": sub["raw_fwd_bias_bp"].mean(),
        "mkt_mean_bp":     sub["mkt_bp"].mean(),
        "mod_mean_bp":     sub["sigma_pay_bp"].mean(),
    }

# Console summary
print(f"\n{'Cell':>9}  {'MAE':>7}  {'RMSE':>7}  {'Bias':>8}  {'Fwd bias':>10}  {'Raw bias':>10}  {'N':>5}")
print("-" * 72)
for e, t in CELLS:
    s = cell_stats(e, t)
    if s:
        print(f"  {e}Yx{t}Y  {s['mae_bp']:>7.1f}  {s['rmse_bp']:>7.1f}  "
              f"{s['bias_bp']:>+8.1f}  {s['fwd_bias_bp']:>+10.1f}  "
              f"{s['raw_fwd_bias_bp']:>+10.1f}  {s['n']:>5}")

overall_mae = df["vol_error_bp"].abs().mean()
print(f"\nOverall MAE (CV):       {overall_mae:.1f} bp")
print(f"Mean fwd bias (CV):     {df['forward_bias_bp'].mean():+.1f} bp  (target: 0)")
print(f"Mean raw fwd bias:      {df['raw_fwd_bias_bp'].mean():+.1f} bp  (uncorrected)")
...
lines = [
    r"\begin{table}[H]",
    r"\centering",
    (r"\caption{Per-cell ATM vol errors: $\Lambda$-MPR model with mean-correction "
     r"control variate (ep\,999, EUR test set). The MC paths are shifted so that "
     r"$\hat{E}^A[F_T] = F_0$ exactly. Forward bias is zero by construction; "
     r"raw bias (pre-correction) is shown for reference.}"),
    r"\label{tab:lambda_per_cell_cv}",
    r"\small",
    r"\begin{tabular}{@{}ccrrrrrr@{}}",
    r"\toprule",
    (r"\textbf{Exp} & \textbf{Ten} & \textbf{Mkt (bp)} & "
     r"\textbf{Mod (bp)} & \textbf{MAE (bp)} & \textbf{RMSE (bp)} & "
     r"\textbf{Fwd bias} & \textbf{Raw bias} \\"),
    r"\midrule",
]
    ...
    for t in TENOR_VALS:
        ...
        if s is None:
            ...
            lines.append(
                ...
                f"& {s['mae_bp']:.1f} & {s['rmse_bp']:.1f} "
                f"& {s['fwd_bias_bp']:+.1f} & {s['raw_fwd_bias_bp']:+.1f} \\\\"
            )
...
with open(os.path.join(OUT_DIR, "tab_lambda_per_cell.tex"), "w") as f:
    f.write("\n".join(lines))
print("Saved: tab_lambda_per_cell.tex")
...
fig.suptitle(r"$\Lambda$-MPR + CV: Model vs Market ATM Vol, EUR test set",
...
fig.suptitle(r"$\Lambda$-MPR + CV: Vol error (model $-$ market) over time, EUR test set",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0, 0, 1, 0.96])
...
ax.set_title(r"$\Lambda$-MPR + CV: Vol MAE per Cell (bp)", fontsize=10)
plt.tight_layout()
...
# ================================================================
# Figure 4: Forward bias after correction — should be ~0
# ================================================================
...
for i, e in enumerate(EXPIRY_VALS):
    for j, t in enumerate(TENOR_VALS):
        ...
        ax.text(0.04, 0.96, f"mean={mean_bias:+.1f}bp",
                transform=ax.transAxes, fontsize=7, va="top",
                bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))

fig.suptitle(r"$\Lambda$-MPR + CV: Forward bias after mean-correction (target: 0 bp)",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0, 0, 1, 0.96])
...
# ================================================================
# Figure 5: Raw (pre-correction) forward bias — diagnostic
# ================================================================
...
for i, e in enumerate(EXPIRY_VALS):
    for j, t in enumerate(TENOR_VALS):
        ...
        if len(sub) == 0:
            ax.set_visible(False)
            continue
        ax.plot(sub["date"], sub["raw_fwd_bias_bp"], color="#ea580c", lw=1.0, alpha=0.8)
        ax.axhline(0, color="black", lw=0.8, ls="--")
        ax.set_title(f"{e}Yx{t}Y", fontsize=9)
        ax.set_ylabel("Raw fwd bias (bp)", fontsize=7)
        ax.tick_params(labelsize=6)
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
        ax.xaxis.set_major_locator(mdates.YearLocator(2))
        plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, ha="right")
        mean_bias = sub["raw_fwd_bias_bp"].mean()
        ax.text(0.04, 0.96, f"mean={mean_bias:+.0f}bp",
                transform=ax.transAxes, fontsize=7, va="top",
                bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.7))

fig.suptitle(r"$\Lambda$-MPR: Raw forward bias (pre-correction), EUR test set",
             fontsize=11, fontweight="bold")
plt.tight_layout(rect=[0, 0, 1, 0.96])
fig.savefig(os.path.join(OUT_DIR, "fig_raw_bias_timeseries.png"),
            dpi=150, bbox_inches="tight")
plt.close(fig)
print("Saved: fig_raw_bias_timeseries.png")

print(f"\nAll outputs written to: {OUT_DIR}")

print("\nSummary:")
print(f"  Overall MAE (CV):    {overall_mae:.1f} bp")
print(f"  Fwd bias (CV):       {df['forward_bias_bp'].mean():+.1f} bp  (0 by construction)")
print(f"  Raw fwd bias:        {df['raw_fwd_bias_bp'].mean():+.1f} bp  (uncorrected)")
