# ==================== Daily Vol-Level MPR Evaluation ====================
"""
Evaluation for Training_daily_vol_mpr.py checkpoint.

sigma_eff(e,n,z0,vt) = exp( base + expiry_off[e] + tenor_off[n]
                           + delta*tanh(W@z0) + d_vol*v_t )

v_t = leave-one-cell-out average market vol on date t (bp).

Output -> Figures/pricing/eval_daily_vol_mpr/
"""
...
LM_CKPT       = os.path.join(PROJECT_ROOT, "Figures", "TrainingResults",
                              f"dim{LATENT_DIM}_daily_vol_mpr", "ep1000",
                              "checkpoint_daily_vol_mpr_ep1000.pt")
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_daily_vol_mpr")
...
# ── module definitions (must match Training_daily_vol_mpr.py) ──────────────────
...
class DailyVolMPR(nn.Module):
    ...
    def get_sigma_eff(self, expiry, tenor, z0, v_t):
        ...
        return (self.log_sigma_base + self.log_sigma_expiry[e]
                + self.log_sigma_tenor[n] + regime + self.d_vol * v_t).exp()
...
class DailyVolDriftWrapper(nn.Module):
    def __init__(self, model, expiry, tenor, z0, v_t):
        super().__init__()
        self.model = model; self.expiry = expiry; self.tenor = tenor
        self._z0 = z0; self._v_t = v_t

    def forward(self, z_t): return self.model.drift(z_t, self.expiry)

    @property
    def sigma_vec(self): return self.model.get_sigma_eff(self.expiry, self.tenor, self._z0, self._v_t)
...
with torch.no_grad():
    ...
    print(f"delta     = {lm.delta.numpy().round(4)}")
...
# Precompute LOO vol averages (same as training)
loo_vol = {}
...
print(f"EUR: {len(meta_eur)} curve dates, {len(all_dates)} vol dates  |  {len(loo_vol)} LOO entries")
...
print(f"d_vol = {lm.d_vol.detach().numpy().round(4)}")
...
for split_label2, split_key2, label_suffix in [("all dates",None,"all"),("test set","test","test"),("train set","train","train")]:
    lines = [r"\begin{table}[H]", r"\centering",
             rf"\caption{{Per-cell vol errors: daily vol-level MPR ({split_label2}). EUR.}}",
             rf"\label{{tab:daily_vol_mpr_per_cell_{label_suffix}}}",
    ...
    fname = f"tab_daily_vol_mpr_per_cell_{label_suffix}.tex"
...
fig.suptitle("Daily Vol-Level MPR: Vol error (amber=test, CALIBRATION)", fontsize=11, fontweight="bold")
...
for split_label3, split_key3, fname3 in [("all dates",None,"fig_vol_heatmap.png"),("test set","test","fig_vol_heatmap_test.png"),("train set","train","fig_vol_heatmap_train.png")]:
    ...
    ax.set_title(f"Daily Vol MPR (CALIBRATION): MAE — {split_label3}")
    plt.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, fname3), dpi=150, bbox_inches="tight")
    plt.close(fig); print(f"Saved: {fname3}")

# v_t vs vol_error scatter (shows whether LOO vol is helpful)
fig, ax = plt.subplots(figsize=(7, 4))
ax.scatter(df["v_t_bp"], df["vol_error_bp"], s=4, alpha=0.4, color="#0891b2")
ax.axhline(0, color="black", lw=0.7, ls="--")
ax.set_xlabel("LOO market vol level v_t (bp)"); ax.set_ylabel("Vol error (bp)")
ax.set_title("Residual error vs market vol level — calibration model")
plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "fig_residual_vs_vt.png"), dpi=150, bbox_inches="tight")
plt.close(fig); print("Saved: fig_residual_vs_vt.png")

print(f"\nAll outputs: {OUT_DIR}")
print(f"Overall MAE: {overall_mae:.1f} bp")
print(f"d_vol = {lm.d_vol.detach().numpy().round(4)}")
print("REMINDER: This is a calibration model — v_t uses option-market information.")
