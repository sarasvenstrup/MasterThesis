# ==================== Expiry-Conditional MPR Pricing Calibration ====================
"""
Expiry-Conditional Market Price of Risk (MPR) pricing adjustment.

Extends Constant MPR by learning a separate drift-correction vector per expiry bucket:

    K_price(z_t) = K_base(z_t) + L_base(z_t) @ lambda_e

where lambda_e in R^d is selected by the option expiry e in {1Y, 5Y, 10Y}.

Key properties:
  - No feedback loop: lambda_e is constant during each simulation (does not depend on z_t)
  - Warm-started: all lambda_e initialised to the Constant MPR lambda_0, then free to diverge
  - Strictly more expressive than Constant MPR (reduces to it when all lambda_e are equal)

Trainable: lambda_expiry [3, d] + log_sigma_vec [d] = 3d + d = 16 params (d=4)

Output: Figures/TrainingResults/dim4_expiry_mpr/ep{EPOCHS}/
"""
...
# ── settings ───────────────────────────────────────────────────────────────────
LATENT_DIM   = 4
EXPIRY_VALS  = [1, 5, 10]   # buckets — one lambda per bucket
...
# Warm-start all lambda_e from Constant MPR lambda_0
...
LAMBDA_VOL  = 1.0
LAMBDA_BIAS = 0.5
LAMBDA_L2   = 1e-3   # L2 on all lambda_e
...
FIGURES_DIR = os.path.join(
    PROJECT_ROOT, "Figures", "TrainingResults",
    f"dim{LATENT_DIM}_expiry_mpr", f"ep{EPOCHS}"
)
os.makedirs(FIGURES_DIR, exist_ok=True)

# ── module ─────────────────────────────────────────────────────────────────────

class ExpiryMPRAdjustment(nn.Module):
    """
    Expiry-conditional Market Price of Risk pricing adjustment.

    K_price(z_t) = K_base(z_t) + L_base(z_t) @ lambda_e

    where lambda_e is a per-expiry learnable vector selected by option expiry.
    No feedback loop: lambda_e does not depend on z_t.

    Trainable parameters:
      lambda_expiry  [n_expiry, d]  one vector per expiry bucket
      log_sigma_vec  [d]            per-factor diffusion scales (shared)
    Total: n_expiry * d + d = 3*4 + 4 = 16 (for d=4, n_expiry=3)
    """

    def __init__(self, kp_module, h_module, latent_dim, expiry_vals):
        super().__init__()
        self.kp          = kp_module
        self.h           = h_module
    ...
    def get_lambda(self, expiry: int) -> torch.Tensor:
        """Return the lambda vector for a given expiry (integer years)."""
        idx = self.expiry_to_idx[expiry]
        return self.lambda_expiry[idx]   # [d]

    def drift(self, z_t, expiry: int) -> torch.Tensor:
        """Full risk-neutral drift for a given expiry bucket."""
        k_base       = self.kp(z_t)
        sigmas, rhos = self.h(z_t)
        L            = L_from_sigmas_rhos(sigmas, rhos, validate=False)
        lam          = self.get_lambda(expiry).unsqueeze(0).expand(z_t.shape[0], -1)
        return k_base + torch.einsum('bij,bj->bi', L, lam)

    def forward(self, z_t):
        """Fallback: average of all lambdas (not used during training)."""
        k_base       = self.kp(z_t)
        sigmas, rhos = self.h(z_t)
        L            = L_from_sigmas_rhos(sigmas, rhos, validate=False)
        lam_mean     = self.lambda_expiry.mean(0).unsqueeze(0).expand(z_t.shape[0], -1)
        return k_base + torch.einsum('bij,bj->bi', L, lam_mean)
...
class ExpiryDriftWrapper(nn.Module):
    """Wraps ExpiryMPRAdjustment with a fixed expiry, matching k_override interface."""

    def __init__(self, expiry_mpr, expiry: int):
        super().__init__()
        self.expiry_mpr = expiry_mpr
        self.expiry     = expiry

    def forward(self, z_t):
        return self.expiry_mpr.drift(z_t, self.expiry)

    @property
    def sigma_vec(self):
        return self.expiry_mpr.sigma_vec
...
def compute_pricing_loss(
    ...
    for _, row in sample.iterrows():
        ...
        if date not in date_to_idx or expiry not in lm.expiry_to_idx:
            continue
        ...
        drift_wrapper = ExpiryDriftWrapper(lm, expiry)
        ...
        try:
            ...
            z_keep = z_T[mask]
            # Use pricing dynamics so grad flows through sigma_vec correctly
...
lm = ExpiryMPRAdjustment(model.K, model.H, LATENT_DIM, EXPIRY_VALS).to(device)
...
if os.path.exists(CMPR_CKPT):
    ...
    with torch.no_grad():
        if "lambda_0" in cmpr_state:
            lam0 = cmpr_state["lambda_0"].to(device)
            # Initialise all expiry lambdas to the same constant MPR value
            for i in range(len(EXPIRY_VALS)):
                lm.lambda_expiry[i].copy_(lam0)
        if "log_sigma_vec" in cmpr_state:
            lm.log_sigma_vec.copy_(cmpr_state["log_sigma_vec"].to(device))
    print(f"Warm-started from: {CMPR_CKPT}")
    for i, e in enumerate(EXPIRY_VALS):
        print(f"  lambda_{e}Y = {lm.lambda_expiry[i].detach().cpu().numpy().round(4)}")
    print(f"  sigma_vec  = {lm.sigma_vec.detach().cpu().numpy().round(4)}")
else:
    print(f"WARNING: Constant MPR checkpoint not found at {CMPR_CKPT}")
    print("  Starting from scratch (all lambda_e = 0, sigma_vec ~ 0.165)")

n_params = sum(p.numel() for p in lm.parameters() if p.requires_grad)
print(f"Trainable params: {n_params}  "
      f"(lambda_expiry {len(EXPIRY_VALS)}x{LATENT_DIM} + log_sigma_vec {LATENT_DIM})")

model.train()

optim = torch.optim.Adam([
    {'params': [lm.lambda_expiry], 'lr': LR,                 'name': 'lambda_expiry'},
...
df_vol = df_vol[df_vol["option_maturity"].isin(EXPIRY_VALS)]   # only train on known expiries
...
csv_path  = os.path.join(FIGURES_DIR, f"train_expiry_mpr_log_dim{LATENT_DIM}_ep{EPOCHS}.csv")

# Per-expiry lambda norm columns
lam_cols = [f"lam_{e}Y_norm" for e in EXPIRY_VALS] + \
           [f"lam_{e}Y_v{k+1}" for e in EXPIRY_VALS for k in range(LATENT_DIM)]
csv_cols = (
    ["epoch", "time_total_sec", "time_interval_sec",
     ...
     "gnorm_lam", "gnorm_scale", "lr",
     "sigma_scale_mean"]
    + [f"sigma_s{k+1}" for k in range(LATENT_DIM)]
    + lam_cols
    + ["fwd_bias_diag_bp"]
...
run_config = {
    "version":               "expiry_mpr",
    "description":           "K_price(z_t) = K_base(z_t) + L(z_t) @ lambda_e  (per expiry bucket)",
    "seed":                  SEED,
    "latent_dim":            LATENT_DIM,
    "expiry_vals":           EXPIRY_VALS,
...
hist = {k: [] for k in [
    "vol", "bias", "l2",
    "swp_priced", "path_finite", "sigma_scale",
] + [f"lam_{e}Y_norm" for e in EXPIRY_VALS]}

t0         = time.perf_counter()
t_last_log = t0

print("\n" + "=" * 100)
print("EXPIRY-CONDITIONAL MPR PRICING CALIBRATION")
print("  K_price(z_t) = K_base(z_t) + L(z_t) @ lambda_e   (e in {1Y, 5Y, 10Y})")
print(f"  Trainable: lambda_expiry ({len(EXPIRY_VALS)}x{LATENT_DIM}) "
...
for epoch in range(EPOCHS):
    ...
    for step in range(N_STEPS_PER_EPOCH):
        ...
        loss_l2    = LAMBDA_L2 * lm.lambda_expiry.pow(2).sum()
        ...
        torch.nn.utils.clip_grad_norm_([lm.lambda_expiry], max_norm=2.0)
    ...
    for k, v in [("vol", ep_vol), ("bias", ep_bias), ("l2", ep_l2),
        ...
        hist[k].append(v)
    for i, e in enumerate(EXPIRY_VALS):
        hist[f"lam_{e}Y_norm"].append(lam_norms[i])
    ...
    lam_now = lm.lambda_expiry.detach().cpu()
    row = {
        ...
        "gnorm_lam": gn_lam, "gnorm_scale": gn_scale, "lr": lr_now,
        "sigma_scale_mean": scale_now,
        "fwd_bias_diag_bp": mean_bias_diag,
    }
    for k in range(LATENT_DIM):
        row[f"sigma_s{k+1}"] = float(sigma_vec_now[k])
    for i, e in enumerate(EXPIRY_VALS):
        row[f"lam_{e}Y_norm"] = lam_norms[i]
        for k in range(LATENT_DIM):
            row[f"lam_{e}Y_v{k+1}"] = float(lam_now[i, k])
    for c in ccy_order:
        row[f"rmse_bps_{c}"] = (
    ...
    if (epoch // max(LOG_EVERY, 1)) % HEADER_EVERY == 0:
        lam_header = "  ".join(f"|l{e}Y|" for e in EXPIRY_VALS)
        print(
            f"\n{'ep':>5} {'vol':>10} {'bias':>9} {'l2':>8} "
            f"{'swp%':>5} {'pth%':>5} {'recon':>7} "
            f"{lam_header}  {'sv_mean':>7} "
            f"{'bias_bp':>8} {'gn_lam':>7} {'lr':>8} {'t/ep':>6} {'ETA':>8}  diag"
        )
        print("-" * 160)
    ...
    print(
        ...
        f"{avg_rmse_bps:>7.2f} "
        f"{lam_str}  {scale_now:>7.4f} "
        f"{mean_bias_diag:>+8.1f} {gn_lam:>7.2e} {lr_now:>8.2e} "
        f"{dt_ep:>5.1f}s {eta_str:>8}  {diag_str}"
    )

    if (epoch + 1) % SAVE_EVERY == 0 or epoch == EPOCHS - 1:
        ckpt = os.path.join(FIGURES_DIR, f"checkpoint_expiry_mpr_ep{epoch+1}.pt")
        lam_state = {f"lambda_{e}Y": lm.lambda_expiry[i].detach().cpu()
                     for i, e in enumerate(EXPIRY_VALS)}
        torch.save({
            ...
            **lam_state,
            ...
            "lambda_norms":    lam_norms,
            "latent_dim":      LATENT_DIM,
            "expiry_vals":     EXPIRY_VALS,
            "epoch":           epoch + 1,
            "variant":         config.VARIANT,
        }, ckpt)
        lam_str2 = "  ".join(f"|l{e}Y|={n:.4f}" for e, n in zip(EXPIRY_VALS, lam_norms))
        print(f"  -> checkpoint ep{epoch+1}  {lam_str2}  "
...
torch.save({
    "lm_state_dict":   lm.state_dict(),
    **{f"lambda_{e}Y": lm.lambda_expiry[i].detach().cpu() for i, e in enumerate(EXPIRY_VALS)},
    "log_sigma_vec":   lm.log_sigma_vec.detach().cpu(),
    "sigma_vec":       lm.sigma_vec.detach().cpu(),
    "lambda_norms":    lam_norms,
    "latent_dim":      LATENT_DIM,
    ...
    "epochs":          EPOCHS,
    "variant":         config.VARIANT,
}, os.path.join(FIGURES_DIR, f"checkpoint_expiry_mpr_ep{EPOCHS}.pt"))
...
axes[0].set_title("Expiry MPR: Loss Components"); axes[0].legend()
...
axes[2].set_xlabel("Epoch"); axes[2].set_title("Diffusion Scale"); axes[2].legend()
axes[2].grid(True, alpha=0.3)

colors_exp = ["#2563eb", "#16a34a", "#dc2626"]
for i, e in enumerate(EXPIRY_VALS):
    axes[3].plot(hist[f"lam_{e}Y_norm"], lw=1.2, color=colors_exp[i], label=f"||lambda_{e}Y||")
axes[3].set_xlabel("Epoch"); axes[3].set_title("Per-Expiry Lambda Norms"); axes[3].legend()
axes[3].grid(True, alpha=0.3)

fig.tight_layout()
fig.savefig(os.path.join(FIGURES_DIR, f"expiry_mpr_loss_dim{LATENT_DIM}_ep{EPOCHS}.png"), dpi=200)
plt.close(fig)

print("\n" + "=" * 90)
print("EXPIRY-CONDITIONAL MPR CALIBRATION COMPLETE")
print("=" * 90)
