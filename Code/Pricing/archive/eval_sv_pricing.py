# ==================== SV Pricing MPR Evaluation ====================
"""
Evaluation for Training_sv_pricing.py checkpoint.

CIR stochastic-vol pricing with state-conditioned (z_0) static σ_eff
and state-conditioned initial variance v_0(z_0).

Output -> Figures/pricing/eval_sv_pricing/
"""
...
LM_CKPT       = os.path.join(PROJECT_ROOT, "Figures", "TrainingResults",
                              f"dim{LATENT_DIM}_sv_pricing", "ep1000",
                              "checkpoint_sv_pricing_ep1000.pt")
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_sv_pricing")
...
# ── module definitions (must match Training_sv_pricing.py) ────────────────────
...
def price_cell(date, expiry, tenor):
    ...
    with torch.no_grad():
        ...
        z_T, v_T, D_T = simulate_sv_to_expiry_differentiable(
            model, lm.cir, z0,
            n_steps=n_steps, dt=dt_eff, n_paths=N_PATHS,
            eps_z=eps_z, eps_v=eps_v,
            k_override=wrapper, sigma_scale=wrapper.sigma_vec,
            v0=None, antithetic=True, freeze_H=True)
    ok = torch.isfinite(z_T).all(1) & torch.isfinite(D_T) & torch.isfinite(v_T)
    if ok.sum() < 16: return None
    z_k, D_k, v_k = z_T[ok], D_T[ok], v_T[ok]

    v_T_mean    = float(v_k.mean())
    sv_factor_T = math.sqrt(v_T_mean / float(lm.cir.theta))
    sigma_scale_T = wrapper.sigma_vec * sv_factor_T

    with torch.no_grad():
        _, aux_T = model.decode_from_z(z_k, tau=None, return_aux=True,
                                       k_override=wrapper, sigma_scale=sigma_scale_T)
    P_T = aux_T["P_full"]
    ...
    return {
        ...
        "v_T_mean":        v_T_mean,
        "v_T_std":         float(v_k.std()),
        "sv_factor":       sv_factor_T,
        "v_0":             float(lm.cir.initial_v(z0).detach()),
    }
...
for counter, (_, row) in enumerate(combos.iterrows()):
    ...
    rows.append({"date": date, "expiry": expiry, "tenor": tenor,
                 ...
                 "v_0": result["v_0"],
                 "v_T_mean": result["v_T_mean"],
                 "v_T_std": result["v_T_std"],
                 "sv_factor": result["sv_factor"]})
...
for split_label2, split_key2, label_suffix in [("all dates",None,"all"),("test set","test","test"),("train set","train","train")]:
    lines = [r"\begin{table}[H]", r"\centering",
             rf"\caption{{Per-cell vol errors: SV pricing MPR ({split_label2}). EUR.}}",
             rf"\label{{tab:sv_pricing_per_cell_{label_suffix}}}",
    ...
    fname = f"tab_sv_pricing_per_cell_{label_suffix}.tex"
...
fig.suptitle("SV Pricing MPR: Vol error (amber=test)", fontsize=11, fontweight="bold")
...
for split_label3, split_key3, fname3 in [("all dates",None,"fig_vol_heatmap.png"),("test set","test","fig_vol_heatmap_test.png"),("train set","train","fig_vol_heatmap_train.png")]:
    ...
    ax.set_title(f"SV Pricing MPR: MAE — {split_label3}")
    plt.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, fname3), dpi=150, bbox_inches="tight")
    plt.close(fig); print(f"Saved: {fname3}")

# v_0(z_0) and v_T over time — to see if the model learned a vol-regime indicator
fig, ax = plt.subplots(figsize=(10, 4))
df_dates = df.sort_values("date")
# Plot one trace per (expiry, tenor) cell as scatter colored by expiry
colors_e = ["#2563eb","#16a34a","#dc2626"]
for i, e in enumerate(EXPIRY_VALS):
    sub = df_dates[df_dates["expiry"]==e]
    if len(sub):
        ax.scatter(sub["date"], np.sqrt(sub["v_0"]), s=3, color=colors_e[i],
                   alpha=0.4, label=f"√v_0 ({e}Y expiry)")
theta = float(lm.cir.theta)
ax.axhline(math.sqrt(theta), color="black", lw=1, ls="--", label=f"√θ={math.sqrt(theta):.4f}")
ax.set_xlabel("Date"); ax.set_ylabel("√v_0")
ax.set_title("State-conditioned initial vol level √v_0(z_0) over time")
ax.legend(markerscale=4, fontsize=8); plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "fig_v0_timeseries.png"), dpi=150, bbox_inches="tight")
plt.close(fig); print("Saved: fig_v0_timeseries.png")

# v_T spread (how stochastic was v over the simulation)
fig, ax = plt.subplots(figsize=(10, 4))
ax.scatter(df_dates["date"], np.sqrt(df_dates["v_T_mean"]), s=3, color="#0891b2",
           alpha=0.5, label="√mean(v_T)")
ax.scatter(df_dates["date"], np.sqrt(df_dates["v_T_mean"] + df_dates["v_T_std"]),
           s=1, color="#7c3aed", alpha=0.3, label="√(mean + std)")
ax.axhline(math.sqrt(theta), color="black", lw=1, ls="--", label=f"√θ={math.sqrt(theta):.4f}")
ax.set_xlabel("Date"); ax.set_ylabel("√v_T")
ax.set_title("Terminal vol √v_T over time (mean across paths)")
ax.legend(markerscale=4, fontsize=8); plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "fig_vT_timeseries.png"), dpi=150, bbox_inches="tight")
plt.close(fig); print("Saved: fig_vT_timeseries.png")

print(f"\nAll outputs: {OUT_DIR}")
print(f"Overall MAE: {overall_mae:.1f} bp")
cir_summary = lm.cir.summary()   # re-fetch — `s` was overwritten by cell_stats() in the loop
print(f"CIR: κ={cir_summary['kappa']:.4f}  √θ={cir_summary['sqrt_theta']:.4f}  σ_v={cir_summary['sigma_v']:.4f}")
print(f"v_0 state-cond: α_v={cir_summary['alpha_v']:+.4f}  |w_v|={cir_summary['w_v_norm']:.4f}")
