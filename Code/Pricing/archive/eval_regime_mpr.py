# ==================== Regime MPR Evaluation ====================
"""
Evaluation for Training_regime_mpr.py checkpoint.

K_price(z_t) = K_base(z_t) + L_base(z_t) @ (lambda_0 + A @ z0)
  lambda_0  in R^d  : constant bias
  A         in R^{dxd} : regime-sensitivity matrix
  z0        in R^d  : encoder(today's curve), fixed per observation

Prices all EUR dates x 9 cells and saves per_cell_final.csv + figures.
Output -> Figures/pricing/eval_regime_mpr/
"""
...
LM_CKPT       = os.path.join(PROJECT_ROOT, "Figures", "TrainingResults",
                              f"dim{LATENT_DIM}_regime_mpr", "ep1000",
                              "checkpoint_regime_mpr_ep1000.pt")
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_regime_mpr")
...
# ── modules (must match Training_regime_mpr.py exactly) ───────────────────────

class RegimeMPRAdjustment(nn.Module):
    """K_price(z_t) = K_base(z_t) + L(z_t) @ (lambda_0 + A @ z0)"""
    ...
    def lambda_eff(self, z0):
        ...
        return self.lambda_0.unsqueeze(0) + (z0 @ self.A.T)

    def drift(self, z_t, z0_broadcast):
        """Full risk-neutral drift given current state and fixed initial encoding."""
        with torch.no_grad():
            k_base       = self.kp(z_t)
            sigmas, rhos = self.h(z_t)
            L            = L_from_sigmas_rhos(sigmas, rhos, validate=False)
        lam_eff = self.lambda_eff(z0_broadcast)
        return k_base + torch.einsum('bij,bj->bi', L, lam_eff)

    def forward(self, z_t):
        """Fallback (constant MPR, A=0)."""
        with torch.no_grad():
            k_base       = self.kp(z_t)
            sigmas, rhos = self.h(z_t)
            L            = L_from_sigmas_rhos(sigmas, rhos, validate=False)
...
class RegimeDriftWrapper(nn.Module):
    """Wraps RegimeMPRAdjustment with a fixed z0 for one observation."""
...
def price_cell(date, expiry, tenor):
    ...
    with torch.no_grad():
        z0 = model.encoder(xb)           # [1, d] — fixed for this observation
    ...
    # Also record the effective lambda for this date
    with torch.no_grad():
        lam_eff = lm.lambda_eff(z0).squeeze(0).cpu().numpy()

    return {
        "sigma_str_bp":    sigma_str_bp,
        "forward_bias_bp": forward_bias_bp,
        "path_frac":       path_frac,
        "lam_eff_norm":    float(np.linalg.norm(lam_eff)),
    }
...
for counter, (_, row) in enumerate(combos.iterrows()):
    ...
    rows.append({
        ...
        "lam_eff_norm":    result["lam_eff_norm"],
    })
...
print(f"\nlambda_0  = {l0.round(4)}  (||.||={np.linalg.norm(l0):.4f})")
print(f"||A||_F   = {np.linalg.norm(A, 'fro'):.4f}")
print(f"sigma_vec = {sv.round(4)}  (mean={sv.mean():.4f})")
...
for split_label2, split_key2, label_suffix in [
    ...
    lines = [
        ...
        (rf"\caption{{Per-cell ATM straddle vol errors: regime-conditioned MPR "
         rf"({split_label2}). EUR.}}"),
        rf"\label{{tab:regime_mpr_per_cell_{label_suffix}}}",
        r"\small",
    ...
    fname = f"tab_regime_mpr_per_cell_{label_suffix}.tex"
...
fig.suptitle("Regime MPR: Vol error over time (amber=test)", fontsize=11, fontweight="bold")
...
for split_label3, split_key3, fname3 in [
    ...
    ax.set_title(f"Regime MPR: Vol MAE per Cell (bp), {split_label3}")
    plt.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, fname3), dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {fname3}")

# Lambda_eff norm over time (shows regime variation)
fig, ax = plt.subplots(figsize=(11, 4))
df_lam = df[["date", "lam_eff_norm"]].drop_duplicates("date").sort_values("date")
test_min = df[df["split"] == "test"]["date"].min()
test_max = df[df["split"] == "test"]["date"].max()
ax.axvspan(test_min, test_max, alpha=0.07, color="#f59e0b", label="Test period")
ax.plot(df_lam["date"], df_lam["lam_eff_norm"], color="#7c3aed", lw=1.2,
        label=r"$\|\lambda_{\mathrm{eff}}(z_0)\|$")
ax.axhline(float(np.linalg.norm(l0)), color="grey", lw=0.9, ls="--",
           label=r"$\|\lambda_0\|$ (constant MPR)")
ax.set_ylabel(r"$\|\lambda_{\mathrm{eff}}\|$", fontsize=10)
ax.set_title(r"Regime MPR: Effective $\lambda$ norm over time (amber = test)")
ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
ax.xaxis.set_major_locator(mdates.YearLocator(2))
plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, ha="right")
ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "fig_lam_eff_timeseries.png"), dpi=150, bbox_inches="tight")
plt.close(fig)
print("Saved: fig_lam_eff_timeseries.png")

print(f"\nAll outputs written to: {OUT_DIR}")
print(f"\nFinal summary:")
print(f"  Overall MAE:   {overall_mae:.1f} bp")
print(f"  Fwd bias:      {df['forward_bias_bp'].mean():+.1f} bp")
print(f"  lambda_0:      {l0.round(4)}")
print(f"  ||A||_F:       {np.linalg.norm(A, 'fro'):.4f}")
print(f"  sigma_vec:     {sv.round(4)}")
