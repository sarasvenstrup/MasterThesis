# ==================== Expiry-Conditional MPR Evaluation ====================
"""
Evaluation for Training_expiry_mpr.py checkpoint.

K_price(z_t) = K_base(z_t) + L_base(z_t) @ lambda_e
  lambda_e in R^d : per-expiry constant drift bias (e in {1, 5, 10})

Output -> Figures/pricing/eval_expiry_mpr/
"""
...
LM_CKPT       = os.path.join(PROJECT_ROOT, "Figures", "TrainingResults",
                              f"dim{LATENT_DIM}_expiry_mpr", "ep1000",
                              "checkpoint_expiry_mpr_ep1000.pt")
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_expiry_mpr")
...
# ── modules (must match Training_expiry_mpr.py exactly) ───────────────────────
...
class ExpiryMPRAdjustment(nn.Module):
    ...
    def get_lambda(self, expiry: int) -> torch.Tensor:
        return self.lambda_expiry[self.expiry_to_idx[expiry]]
...
for i, e in enumerate(EXPIRY_VALS):
    lv = lm.lambda_expiry[i].detach().cpu().numpy()
    print(f"lambda_{e}Y = {lv.round(4)}  (||.||={np.linalg.norm(lv):.4f})")
...
def price_cell(date, expiry, tenor):
    ...
    with torch.no_grad():
        # Pass drift_wrapper and sigma_vec so the decoder ODE uses the same
        # pricing dynamics as the simulation (consistency fix).
...
for split_label2, split_key2, label_suffix in [
    ...
    lines = [
        ...
        (rf"\caption{{Per-cell ATM straddle vol errors: expiry-conditional MPR "
         rf"({split_label2}). EUR.}}"),
        rf"\label{{tab:expiry_mpr_per_cell_{label_suffix}}}",
        r"\small",
    ...
    fname = f"tab_expiry_mpr_per_cell_{label_suffix}.tex"
...
# Helper: fill date gaps with NaN so matplotlib doesn't connect across gaps
def gapped(sub):
    """Return (dates, values) with NaN inserted where date jumps > 10 business days."""
    ...
    for i, (_, r) in enumerate(sub.iterrows()):
        if i > 0:
            ...
            if gap > 14:   # more than ~2 weeks = data gap
...
fig.suptitle("Expiry-Conditional MPR: Vol error over time (amber=test)", fontsize=11, fontweight="bold")
...
for split_label3, split_key3, fname3 in [
    ...
    ax.set_title(f"Expiry MPR: Vol MAE (bp) — {split_label3}")
    plt.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, fname3), dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {fname3}")

# Per-expiry lambda values bar chart
