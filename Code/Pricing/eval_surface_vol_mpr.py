# ==================== Surface Vol MPR Evaluation ====================
"""
Evaluation for Training_surface_vol_mpr.py checkpoint.

Uses three precomputed leave-one-cell-out vol features (per-expiry,
per-tenor, global) as inputs to the σ_eff formula.

Output -> Figures/pricing/eval_surface_vol_mpr/
"""
...
RATE_CLIP   = 0.50
ANNUITY_MAX = 50.0
V_FEATURE_SCALE = 100.0   # MUST match Training_surface_vol_mpr.py
...
LM_CKPT       = os.path.join(PROJECT_ROOT, "Figures", "TrainingResults",
                              f"dim{LATENT_DIM}_surface_vol_mpr", "ep1000",
                              "checkpoint_surface_vol_mpr_ep1000.pt")
OUT_DIR       = os.path.join(PROJECT_ROOT, "Figures", "pricing", "eval_surface_vol_mpr")
...
# ── module definitions (must match Training_surface_vol_mpr.py) ────────────────
...
class SurfaceVolMPR(nn.Module):
    ...
    def get_sigma_eff(self, expiry, tenor, z0, v_t_e, v_t_n, v_t_g):
...
class SurfaceVolDriftWrapper(nn.Module):
    def __init__(self, model, expiry, tenor, z0, v_t_e, v_t_n, v_t_g):
        super().__init__()
        self.model = model; self.expiry = expiry; self.tenor = tenor
        self._z0 = z0; self._v_e = v_t_e; self._v_n = v_t_n; self._v_g = v_t_g
...
# Build LOO features (same as training)
loo_features = {}
for date, grp in df_vol.groupby("as_of_date"):
    ...
    for _, r in grp.iterrows():
        ...
        if len(same_e_other_t) == 0 or len(same_t_other_e) == 0 or len(other_both) == 0:
            continue
        # MUST match training: features rescaled by V_FEATURE_SCALE
        ...
        loo_features[(date_ts, e, t)] = (v_e, v_n, v_g)

print(f"LOO features: {len(loo_features)} cell-date entries")
...
for counter, (_, row) in enumerate(combos.iterrows()):
    ...
    rows.append({"date": date, "expiry": expiry, "tenor": tenor,
                 ...
                 "v_e": result["v_e"], "v_n": result["v_n"], "v_g": result["v_g"]})
...
for split_label2, split_key2, label_suffix in [("all dates",None,"all"),("test set","test","test"),("train set","train","train")]:
    lines = [r"\begin{table}[H]", r"\centering",
             rf"\caption{{Per-cell vol errors: surface vol MPR ({split_label2}). EUR.}}",
             rf"\label{{tab:surface_vol_mpr_per_cell_{label_suffix}}}",
    ...
    fname = f"tab_surface_vol_mpr_per_cell_{label_suffix}.tex"
...
fig.suptitle("Surface Vol MPR: Vol error (amber=test)", fontsize=11, fontweight="bold")
...
for split_label3, split_key3, fname3 in [("all dates",None,"fig_vol_heatmap.png"),("test set","test","fig_vol_heatmap_test.png"),("train set","train","fig_vol_heatmap_train.png")]:
    ...
    ax.set_title(f"Surface Vol MPR: MAE — {split_label3}")
    plt.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, fname3), dpi=150, bbox_inches="tight")
    plt.close(fig); print(f"Saved: {fname3}")

# Residual vs LOO features
fig, axes = plt.subplots(1, 3, figsize=(13, 4))
for ax, col, label in zip(axes, ["v_e","v_n","v_g"], ["v_t^e (per-expiry)","v_t^n (per-tenor)","v_t^g (global)"]):
    ax.scatter(df[col], df["vol_error_bp"], s=4, alpha=0.4, color="#0891b2")
    ax.axhline(0, color="black", lw=0.7, ls="--")
    ax.set_xlabel(f"{label} (bp)"); ax.set_ylabel("Vol error (bp)")
    ax.set_title(f"Residual vs {label}")
plt.tight_layout()
fig.savefig(os.path.join(OUT_DIR, "fig_residual_vs_loo.png"), dpi=150, bbox_inches="tight")
plt.close(fig); print("Saved: fig_residual_vs_loo.png")

print(f"\nAll outputs: {OUT_DIR}")
print(f"Overall MAE: {overall_mae:.1f} bp")
print(f"|d_expiry|={float(lm.d_expiry.norm()):.4f}  |d_tenor|={float(lm.d_tenor.norm()):.4f}  |d_global|={float(lm.d_global.norm()):.4f}")
