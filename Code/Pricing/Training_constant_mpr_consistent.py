# ============== Constant MPR Pricing Calibration (no anchoring) ===============
"""
Constant Market Price of Risk (MPR) pricing adjustment — non-anchored variant.
...
is internally consistent.  z_0 is taken straight from the encoder, so
S^*(E(S_0)) is allowed to drift away from the market curve by the
encoder/decoder mismatch under the shifted dynamics — the anchor RMSE is
only TRACKED as a diagnostic, not penalised in the loss.

The anchored counterpart (Training_constant_mpr_anchored.py) is identical
except that it solves an inverse-decoder problem for z_0^* so that
S^*(z_0^*) ≈ S_0^{mkt} before each pricing step.
...
Output: Figures/TrainingResults/dim4_constant_mpr_consistent/ep{EPOCHS}/
"""
...
FIGURES_DIR = os.path.join(
    PROJECT_ROOT, "Figures", "TrainingResults",
    f"dim{LATENT_DIM}_constant_mpr_consistent", f"ep{EPOCHS}"
)
...
def compute_pricing_loss(
    ...
    for _, row in sample.iterrows():
        ...
        # Initial latent state: z0 = E(S_0).  P(0, .) under the SHIFTED
        # decoder for full internal consistency with simulation and the
        # terminal decode.  No re-anchoring is performed — the resulting
        # anchor RMSE is tracked only as a diagnostic.
        with torch.no_grad():
            z0 = model.encoder(xb)
...
run_config = {
    "version":               "constant_mpr_consistent",
