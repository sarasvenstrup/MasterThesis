...
def plot_L_on_date(
    ...
    # For d=2: entries are L11, L21, L22 (L12 is 0 for lower triangular if you enforce it)
    ...
    # --- 3) Print matrices (optional, useful when one currency is crazy)
...
# ---------- run it ----------
# Pick the same date you used in your SR plots
paper_date = pd.to_datetime("2016-08-30")
date_pick_L = paper_date if (meta_eval["as_of_date"] == paper_date).any() else meta_eval["as_of_date"].iloc[0]
...
sigma_L, meta_used = plot_L_on_date(
    ...
    meta_df=meta_eval,
    currency_colors=currency_color_map,
    date_pick=date_pick_L,
    figures_dir=FIGURES_DIR,   # or None if you don't want saving
)