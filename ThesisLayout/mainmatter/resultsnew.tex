...
\begin{figure}[ht]
  ...
  \caption{Average squared cosine similarity (in \%) between the encoder weight
    vectors and each principal component direction of the in-sample swap rate
    curve, for latent dimensions $\ell \in \{2,3,4\}$. Each bar shows
    $\frac{1}{\ell}\sum_{k=1}^{\ell} w_{kj}$, where $w_{kj}$ is the fraction of
    factor $k$'s weight pointing in the direction of PC$j$. Following the
    approach of \citet{Poulsen2024}.}
