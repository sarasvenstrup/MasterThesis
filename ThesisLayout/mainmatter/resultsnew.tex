...
\begin{table}[ht]
\centering
\caption{Cosine similarity between each encoder weight vector $\mathbf{w}_k$
  and the principal component directions of the in-sample swap rate curve,
  for latent dimensions $\ell \in \{2,3,4\}$. A value close to $\pm 1$ on
  the diagonal indicates that the factor aligns with the corresponding PC.}
\label{tab:weight_projection}
\pgfplotstabletypeset[
  col sep=comma,
  columns={model,factor,PC1,PC2,PC3,PC4},
  columns/model/.style={string type, column name={Model}},
  columns/factor/.style={string type, column name={Factor}},
  columns/PC1/.style={fixed, precision=3, zerofill, empty cells with={}, column name={PC1}},
  columns/PC2/.style={fixed, precision=3, zerofill, empty cells with={}, column name={PC2}},
  columns/PC3/.style={fixed, precision=3, zerofill, empty cells with={}, column name={PC3}},
  columns/PC4/.style={fixed, precision=3, zerofill, empty cells with={}, column name={PC4}},
  every head row/.style={before row=\toprule, after row=\midrule},
  every last row/.style={after row=\bottomrule},
