...
Following \citet{MatinWanPoulsen2025}, we evaluate the in-sample performance of the autoencoder against two criteria: the reconstruction error, measured by the root mean squared error (RMSE) in basis points (bps), and the satisfaction of the no-arbitrage constraint, assessed through the \textcolor{red}{approximate Sharpe ratio (reference or write what we mean by this diagnostic)} of the reconstructed curves. Whereas \citet{MatinWanPoulsen2025} fix the number of latent factors at $\ell=2$, we extend the analysis to $\ell\in\{1,2,3,4\}$, allowing us to examine the trade-off between model complexity and reconstruction accuracy.
...
The model is trained on the in-sample period 2004--2020, following
\citet{MatinWanPoulsen2025}, over 5\,000 epochs. Figure~\ref{fig:training_loss}
shows the average training RMSE across all currencies for $\ell \in \{1,2,3,4\}$.
All three models converge well before epoch 2\,500, after which the loss
curves flatten and no meaningful improvement is observed. However, we can already as you would expect be able to not move forward with $\ell=1$.
