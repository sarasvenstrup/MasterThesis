...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/thesis_results/AutoencoderPerformance/Q4c_per_currency_bar_chart_all_dims.png}
...
\begin{figure}[htbp]
    ...
    \caption{Pearson correlations between latent factors $z_k$ and yield curve components
    (Level, Slope, Curvature) for $\ell \in \{2,3,4\}$, pooled across all in-sample
    observations. Cell values range from $-1$ to $+1$; the diverging colour scale runs
    from negative (dark) through white (zero) to positive (dark).}
