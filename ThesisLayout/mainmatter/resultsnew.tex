...
Following \citet{MatinWanPoulsen2025}, we evaluate the in-sample performance of the autoencoder against two criteria: the reconstruction error, measured by the root mean squared error (RMSE) in basis points (bps), and the satisfaction of the no-arbitrage constraint, assessed through the \textcolor{red}{approximate Sharpe ratio (reference or write what we mean by this diagnostic)} of the reconstructed curves. We first confirm that the models have converged, then assess fit quality across currencies and latent dimneions, before examining where and when the model struggles most. Whereas \citet{MatinWanPoulsen2025} fix the number of latent factors at $\ell=2$, we extend the analysis to $\ell\in\{2,3,4\}$, allowing us to examine the trade-off between model complexity and reconstruction accuracy.
