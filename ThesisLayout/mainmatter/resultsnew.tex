...
% ── Q5b: Weight projection bar plot ─────────────────────────────────────────
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{Figures/thesis_results/AutoencoderPerformance/Q5b_weight_projection_barplot.png}
  \caption{Squared cosine similarity (in \%) between each encoder weight vector
    and the eight principal component directions of the swap rate curve, for
    latent dimensions $\ell \in \{2,3,4\}$. Each bar shows what fraction of
    a factor's weight lies in the direction of a given PC. Bars within each
    panel sum to 100\%.}
