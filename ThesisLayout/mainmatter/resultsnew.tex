...
PCA is applied to decompose the swap rate curve into $M=8$ principal components. Figure \ref{fig:weight_projection_bar} shows the proportion of variance explained by $PC_j$, $\text{PVE}_j=\lambda_j/\sum_{k=1}^M\lambda_k$, where the first two principal components account for over $99\%$ of the total variation in the swap curve data. The remaining components are negligible, motivating the use of a low-dimensional latent representation with $\ell = \{2,3,4\}$ factors.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/thesis_results/AutoencoderPerformance/Q5b_weight_projection_barplot.png}
  \caption{Relative weights of the eigenvalues $\tilde{\lambda}_j = \lambda_j /
...
\begin{figure}[ht]
  ...
  \caption{Left: principal component scores over time. Right: eigenvector loadings $\mathbf{v}_j$ across the eight
  maturities. Both computed from a global PCA fitted on in-sample swap rates
  (2004--2020).}
