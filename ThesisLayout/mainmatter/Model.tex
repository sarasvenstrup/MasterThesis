...
\end{equation}
where the sum runs over all maturities on the grid specified with $\tau_i\leq\tau$. 
...
The network $\mathcal{G}$ takes as input $(z_t^\top,\tau)^\top\in\mathbb{R}^{\ell+1}$ and has one hidden layer of width 10 \citep{MatinWanPoulsen2025}, with neurons $x_{1,\mathcal{G}}^{(1)},\ldots,x_{1,\mathcal{G}}^{(10)}$ as illustrated in Figure \ref{fig:rolf_architecture_details}.
...
For diagnostic purposes, the implementation can also evaluate the residual of
the bond-pricing PDE on the maturity grid. Specifically, we
substitute the solved $A_{z_t}(\tau)$ and $B_{z_t}(\tau)$ into
\eqref{eq:bond_pricing_pde} and divide by $P > 0$, giving the PDE residual
\begin{equation}
R(\tau)
=
-\tilde{r}(z_t)
- \partial_\tau A_{z_t}
+ \mathcal{G}(z_t,\tau)\,\partial_\tau B_{z_t}
+ B_{z_t}(\tau)\!\left(
    \partial_\tau\mathcal{G}
    - (\nabla_{z_t}\mathcal{G})^\top\mu(z_t)
    - \tfrac{1}{2}\mathrm{Tr}\!\left[\sigma\sigma^\top H_{z_t}\mathcal{G}\right]
  \right).
\label{eq:pde_residual}
\end{equation}
If the ODE system \eqref{ODEA}--\eqref{ODEB} is solved exactly, $R(\tau)=0$
for all $\tau$, so deviations from zero reflect numerical error introduced by
