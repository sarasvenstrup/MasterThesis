...
The higher-dimensional models are particularly sensitive to such distributional shifts, as illustrated by the December 2019 window where inverted curves appear in the test set despite being largely absent from the training window, causing the $\ell=4$ model to spike to 182 bps while $\ell=2$ is considerably less affected. This pattern is even more pronounced during the worst windows around 2016 and 2022, where the $\ell=3$ and $\ell=4$ models reach 633 and 692 bps respectively. Overall, the out-of-sample results point to a consistent pattern of overfitting that worsens with the number of latent factors, driven not by the amount of training data but by the mismatch between the curve types seen during training and those encountered at test time.
