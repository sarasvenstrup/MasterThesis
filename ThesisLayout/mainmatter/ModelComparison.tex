...
We need to start off by showing how the the augmented model improves in sample fit very much. But then see how it fails OOS, and then compare all models to then conclude that we can impose resrtictions on the encoder-decoder in multiple ways - and we find that we can do this on the reconstrcution loss way like the augmented modl and then in the model structure itselfs, like we did for stable. and from this we see that it is benefifcal to do it not throught the training error as it does not translate through to good OOS performance. 
