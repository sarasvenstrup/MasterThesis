...
The evaluation in Chapter~\ref{chap:evaluation} establishes that the baseline model struggles with negative-rate curves. Examining the worst-fit curves reveals two distinct failure patterns, shown in 
Figure~\ref{fig:baseline_fit_failure_modes}. For curves that cross from negative short-end rates to positive long-end rates, where the actual curve is roughly symmetric around zero, the baseline 
reconstructs a flat curve near zero. For curves where all or most tenors are negative, the baseline reconstructs a curve sloping in the wrong direction entirely.
...
\begin{figure}[H]
    ...
    and stable ($\ell=4$) models, restricted to curves that are either crossing 
    (first 5 tenors negative with a positive long end) or deeply negative 
    (all or most tenors negative), ranked by baseline RMSE.}
    \label{fig:baseline_fit_failure_modes}
\end{figure}

Both failure patterns suggest that the raw swap rates alone may not provide sufficient shape information for the encoder to produce a reliable latent representation when rates are negative. We hypothesise that this is linked to the linearity of the encoder and the fact that explicit slope and curvature information is absent from the input. The following examples illustrate this hypothesis numerically, though we stress that what follows is speculative rather than a formal result.

Examples?


Motivated by this hypothesis, the augmentation adds level-invariant slope and curvature features to the input, so that the model receives explicit shape information regardless of the rate level. The training objective is extended to penalise mismatch in these features directly, giving the model an explicit incentive to match slope and curvature rather than level alone.

\begin{example}
Consider the EUR swap curve on 2020-10-30, where all tenors are negative with the 1Y rate at $-0.53\%$ and the 10Y rate at $-0.28\%$. The short-end slope feature is
$$f_1 = S_{10\text{Y}} - S_{1\text{Y}} = -0.28\% - (-0.53\%) = 0.25\%.$$
Shifting all rates up by $1\%$ gives $S_{1\text{Y}} = 0.47\%$ and 
$S_{10\text{Y}} = 0.72\%$, yet $f_1 = 0.72\% - 0.47\% = 0.25\%$ remains 
unchanged. The feature therefore carries the same shape information regardless 
of the rate level.
\end{example}
