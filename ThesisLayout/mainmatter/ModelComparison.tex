...
\textcolor{red}{NOTES FOR THIS SECTION: We need to start off by showing how the the augmented model improves in sample fit very much. But then see how it fails OOS, and then compare all models to then conclude that we can impose restrictions on the encoder-decoder in multiple ways - and we find that we can do this on the reconstruction loss way like the augmented model and then in the model structure itselfs, like we did for stable. and from this we see that it is benefifcal to do it not throught the training error as it does not translate through to good OOS performance. }



introduction missing.

In the thesis, we have come across multiple models that we now would like to compare and discuss in the light of how using an encoder-decoder model to learn the underlying dynamics of the state variables driving swap rates in this case. We have so far seen the baseline evaluated in Chapter \ref{chap:evaluation} and the stable model introduced and examined in Chapter \ref{chap:simulation}. In this chapter we introduce a third model that seeks to improve the baseline model by making it fit curve variants better. Lastly, we compare the models and conclude on findings regarding improving encoder-decoder models in sample as well as out of sample.
...
\end{align*}
As shown in Figure~\ref{fig:latent_space_shift}, the upward shift moves $z$ 
further into the dense normal training region, while the downward shift moves it 
into the sparse region near the origin occupied by negative-rate curves, landing 
close to $z^* = (-0.0057, 0.0089)^\top$, the actual negative-rate EUR curve from 
2020-03-31. This confirms that the compression is driven by the rate level: any 
curve shifted toward zero will have its latent representation pushed into a region 
that is sparsely covered during training, regardless of its shape.    
\end{example}
