...
closely reveals two distinct failure patterns, shown in 
Figure~\ref{fig:baseline_fit_failure_modes}. For curves where most tenors are 
negative, the baseline reconstructs a flat curve near zero. For curves that 
cross from negative short-end rates to positive long-end rates, the baseline 
reconstructs a curve sloping in the wrong direction entirely.
...
\begin{figure}[H]
    ...
    ranked by baseline RMSE.}
    \label{fig:baseline_fit_failure_modes}
\end{figure}

Both failure patterns share a common root. The encoder input $\mathbf{S}$ 
contains only raw swap rates, which conflate the level and shape of the curve. 
For a positive-rate curve, the level and shape vary together in a way the 
encoder has been trained on. For a negative-rate curve, the raw rates partially 
cancel in the linear projection, and the resulting latent representation no 
longer carries reliable shape information. The slope and curvature of the curve 
are not explicitly represented anywhere in the input.

The augmentation addresses this directly. By adding level-invariant slope and 
curvature features to the input, the model receives explicit shape information 
that is unaffected by whether the rate level is positive or negative. A 
short-end slope of $f_1 = S_{10\text{Y}} - S_{1\text{Y}} = 0.5\%$ carries the 
same information regardless of whether rates are at $2\%$ or $-1\%$. The 
training objective is extended to penalise mismatch in these features directly, 
giving the model an explicit incentive to match slope and curvature rather than 
level alone.
