...
-1.12 & -0.63 & -0.66 & -0.83 & -0.50 & -0.29 & -0.22 & 0.13 \end{pmatrix}.$$ Applying this yields
$$z^{+} = A_1\mathbf{S}^{+}=\begin{pmatrix} -0.0045 \\ -0.0098 \end{pmatrix}, \qquad
z^{-} = A_1\mathbf{S}^{-}=\begin{pmatrix} -0.0068 \\ +0.0186 \end{pmatrix}.$$
The second latent factors have opposite signs. For $\mathbf{S}^{+}$, the small 
positive rates produce small negative products with the negative weights of 
$a_2$, yielding $z_2^{+} = -0.0098$. For $\mathbf{S}^{-}$, the negative rates 
interact with the negative weights to produce positive products that dominate, 
yielding $z_2^{-} = +0.0186$. The second factor is therefore driven by the sign 
and magnitude of the raw rates rather than by the slope of the curve.\circ
\end{example}
