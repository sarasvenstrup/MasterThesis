...
\subsection{Assessing the Augmented Model empirically}

The augmented model is assessed on the same three representative curves used 
to illustrate the baseline failure patterns in 
Figure~\ref{fig:baseline_fit_failure_modes}, with the baseline included as a 
reference. The results are shown in Figure~\ref{fig:failure_modes_all_models}.
...
\begin{figure}[H]
    ...
    \textit{Left:} EUR 2015-03-31, a low but fully positive curve — the baseline 
    fits it well. 
    ...
    and positive long end. 
    RMSE values (bps) are reported as 
...
For the positive-rate EUR curve on 2015-03-31, both models reconstruct the 
curve accurately, confirming that the augmentation does not degrade performance 
on well-behaved curves. For the deeply negative EUR curve on 2020-10-30, the 
augmented model at all dimensions shows a clear change in behaviour relative 
to the baseline, attempting to capture the shape of the curve rather than 
producing a smooth curve in the wrong direction, though the improvement in 
RMSE is most pronounced at $\ell=4$. For the crossing JPY curve on 2016-09-30, 
the augmented model at $\ell=3$ substantially improves the fit, better 
capturing the upward slope, while $\ell=2$ remains close to the baseline and 
$\ell=4$ overshoots at the long end. The in-sample results therefore suggest 
that the augmentation helps, though its effectiveness depends on the latent 
dimension and the specific curve type. It specifically better the curves that have half the tenors that are negative. 


It is again also interesting to look at the OOS fit, where we see that over time we still see that despite the model with three latent factors fit the curves better, OOS it doesnt really help, as it seems to just be a case of overfitting. 


However, the improvement is mainly in-sample. The rolling-window evaluation in Figure~\ref{fig:augmented_rolling_regime_dual_axis} shows that the augmented model still suffers from large out-of-sample errors during periods where the test set contains curve shapes that are underrepresented in the training window.

Resulting in that the augmented input vector helped guide the model in sample on better fittinc curves, but this did not translate will into OOS. 
...
Hereby indicating that again even with the augmented input the latent dimension 2 is the best oos even though yielding an in sample fit that was looked more like the baseline. so having an augmented vector contrubuted IN sample with the need of more latent factors, but failed to genearlize well OOS, proving to not better the baseline.
...
so even though that the augmetned model seemed to fail, it still tells us something about how to improve the construction of the swap rates. we cant just give more information to the encoder-decoder explicitly, but we can help it better modelling wise.
