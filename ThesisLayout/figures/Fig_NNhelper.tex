...
% Central neuron in callout
\node[neuron, fill=red!70, minimum size=18mm] (CN) at (15.5, 1.0) {$\psi_h(\cdot)$};

% Output node in callout
\node[neuron, fill=red!20, minimum size=13mm, font=\small, inner sep=1pt] (CO) at (18.5, 1.0) {$x_h^{(j)}$};

% Weighted arrows inside callout
...
The callout shows how neuron $x_h^{(j)}$ computes its output from the
