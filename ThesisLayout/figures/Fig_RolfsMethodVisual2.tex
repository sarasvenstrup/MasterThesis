...
\caption{Branching architecture of the latent factors $z=(z_1,\ldots,z_\ell)$ into four neural networks producing $\mathcal{G}$, $\mathcal{K}$, $\mathcal{H}$, and $\mathcal{R}$. The networks $\mathcal{G}$, $\mathcal{H}$, and $\mathcal{R}$ contain hidden layers, while $\mathcal{K}(z)$ connects directly to the latent factors without a hidden layer. The function $\mathcal{G}(z,\tau)$ additionally depends on the maturity $\tau$.}
