...
% ======================
\node[neuron, fill=gray!40] (S1)  at (0,  2.5) {$S^{(1)}$};
\node[neuron, fill=gray!40] (S2)  at (0,  1.0) {$S^{(2)}$};
\node at (0, -0.2) {$\vdots$};
\node[neuron, fill=gray!40] (SM)  at (0, -2.2) {$S^{(M)}$};
...
% Reconstructed swaps
% ======================
\node[neuron, fill=gray!10] (St1) at (16,  2.5) {$\tilde{S}^{(1)}$};
\node[neuron, fill=gray!10] (St2) at (16,  1.0) {$\tilde{S}^{(2)}$};
\node at (16, -0.2) {$\vdots$};
\node[neuron, fill=gray!10] (StM) at (16, -2.2) {$\tilde{S}^{(M)}$};
...
\caption{Simplified overview of the encoder--decoder architecture. The encoder 
maps a swap curve $\mathbf{S} = (S^{(1)}, \ldots, S^{(M)})^\top$ with $M$ maturities to $\ell$ latent factors $z = (z_1, \ldots, z_\ell)^\top$. In the decoder, the latent state $z$ is concatenated with a maturity index $\tau_i$ and passed through a hidden layer of width ten to produce a zero-coupon bond price 
$P_{\tau_i}:=P(z, \tau_i)$. This is repeated successively across an annual maturity grid $\tau_i \in \{1Y,2Y, \ldots, 30Y\}$, as indicated by the solid, dashed, and dotted arrows. The resulting bond prices are then converted to the reconstructed swap rates $\widetilde{\mathbf{S}} = (\widetilde{S}^{(1)}, \ldots, 
\widetilde{S}^{(M)})^\top$ at the same maturities as the input, using \eqref{estimatedswaprates}. Note that the figure is a simplification, as computing $P_{\tau_i}$ also relies on additional networks not shown here; see Figure~\ref{fig:rolf_architecture_details}.}
